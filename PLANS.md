...
> - **`cred` — 7 de 12** (branch `item9-b1-adequacao-adapt`): 6 flip limpo + `notify_sla_risk` adaptado; 5 re-xfailados com 2 ACHADOS NOVOS que o desbloqueio do dossie revelou — (a) 3 happy-paths de descredenciamento agora chegam corretamente à cure-window RN-567 (`GW_AguardarNotificacao`/`ICE_PrazoNotificacao`) e ESPERAM: `ST_RegisterDescredenciamento` roda mas o terminal exige dirigir o notification-ack/timer (follow-up de completar o teste, NÃO worker faltando); (b) 2 testes chegam ao notify/check activity mas ainda asseram `has_event(_CRED_PENDED)` — `cred.pended` não é publicado (gap events.publish, separado do dossie).
> - **`pagto` — PENDENTE** (12 sites, `_PAGTO_ALCADA_WORKER_MISSING_REASON`): mesma receita; André serve o dossie de pagto (flow default dele É `pagto_dossier`). Provável mesma estrutura (dead-echo → `ST_<worker> in ended`; possíveis achados downstream ao desbloquear).
