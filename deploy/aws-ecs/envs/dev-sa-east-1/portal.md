...
| `image_digest` | `sha256:` com 64 hex do artefato de aplicação revisado, com módulo portal e dependências travadas. Requer SBOM/assinatura e confiança CA do Aurora verificadas no artefato exato. **O que satisfaz "SBOM/assinatura", em concreto:** `cosign verify` **e** `cosign verify-attestation --type spdxjson` verdes sobre `amh/maezo-operadora@<digest>`, com identidade `https://github.com/Omni-Saude/maezo-operadora/.github/workflows/supply-chain.yml@refs/heads/…` e issuer `https://token.actions.githubusercontent.com` (Sigstore keyless — não há chave). Quem publica é `.github/workflows/supply-chain.yml`; os artefatos ficam no próprio ECR como tags `sha256-<digest>.sig`/`.att`. **Comando exato de verificação, e o que cada parte significa: [`docs/runbooks/supply-chain-imagem.md`](../../../../docs/runbooks/supply-chain-imagem.md).** Assinatura prova origem e integridade do artefato — não ausência de CVE (isso é o portão L3) nem revisão do conteúdo. |
