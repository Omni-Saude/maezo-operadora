...
| `image_digest` | `sha256:` com 64 hex do artefato de aplicação revisado, com módulo portal e dependências travadas. Requer SBOM/assinatura e confiança CA do Aurora verificadas no artefato exato. **O que satisfaz "SBOM/assinatura", em concreto:** `cosign verify` **e** `cosign verify-attestation --type spdxjson` verdes sobre `amh/maezo-operadora@<digest>` com a chave KMS assimétrica da conta, alias fixo `alias/maezo-operadora-dev-image-signing` (assinar: só a role do CI, por `Deny` explícito na policy da chave; verificar: liberado na conta, sem depender de Fulcio/Rekor — o egresso daqui é allowlist de `/32`). Quem publica é o job `assinar` de `.github/workflows/supply-chain.yml`; quem **confere no caminho de entrega** é o job `verificar`, que roda em pull request sobre este tfvars com uma role sem `kms:Sign` — trocar o digest do portal sem assinar o novo reprova o PR. Artefatos no próprio ECR como tags `sha256-<digest>.sig`/`.att`. **Comando exato (o mesmo que o CI roda): `scripts/ci/verify_image_signature.sh <repo>@<digest>`, explicado em [`docs/runbooks/supply-chain-imagem.md`](../../../../docs/runbooks/supply-chain-imagem.md).** Assinatura prova origem e integridade do artefato — não ausência de CVE (isso é o portão L3) nem revisão do conteúdo. |
...
| `engine_schema` | Schema PostgreSQL das tabelas do CIB seven (T1.8b: `cibseven`). Mesmo identificador `^[a-z_][a-z0-9_]{0,62}$`; nunca `public`, `information_schema`, `maezo_native`, `pg_*` nem igual a `native_schema`. Comparado por igualdade exata com o `engine_schema` do manifesto v2 e entra no digest da configuração staff. |
