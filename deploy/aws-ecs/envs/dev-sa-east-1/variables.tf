...
variable "webhook_receiver_image_tag" {
  ...
  EOT
  type        = string
  default     = "193970d2" # 13/09/2026: response-v5 + recusa-v3 (3 categorias) em _respond_llm; ack devolvendo `resposta`/`conversation_id`. O RECEPTOR e quem executa turno; agent-helena segue em c745fd94 ate a proxima promocao
}
...
variable "canal_teste_image_tag" {
  ...
  EOT
  type        = string
  default     = "193970d2" # 13/09/2026: pagina como conversa + canal elevando os campos + recusa de saida
}
