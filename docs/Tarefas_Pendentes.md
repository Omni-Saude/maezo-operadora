...
- [x] R-089 — CONCLUIDO 2026-09-06 (reparo F2/VER-ADR-BATCH, branch `r6/adr-batch`, ainda nao mergeado em `main`): `check_calendar` removido do exemplo de import em `docs/adr/0026-worker-standardization.md:72`; `ans_cron.py:116` re-ancorado para o businessRuleTask `BRT_Calendario`/`decisionRef="ans_calendar"` em `docs/adr/0028-dmn-evaluation-engine-side.md:192` — edicao in-loco autorizada pelo dono, excecao pontual a `docs/adr/README.md:8` so para estas duas linhas; gate `scripts/ci/check_doc_symbol_citations.py` landed no mesmo lote, nao no PR separado `docs/adr-anscron-citation-refresh` (`main` `89a6c29`) previsto originalmente — WP-ADR-RECONCILIACAO — `docs/adr/0026-worker-standardization.md`, `docs/adr/0028-dmn-evaluation-engine-side.md`, `scripts/ci/check_doc_symbol_citations.py`
