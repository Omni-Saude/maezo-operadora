# SUPERSEDED — DO NOT APPLY — DL rows to prepend to docs/decisions-log.md at docs-bundle push (fetch-rebase-retry; adjust numbering if rows landed meanwhile)

> **STAGED-DL-NUMBERING-COLLISION.** As linhas DL-0026..DL-0031 abaixo NUNCA foram aplicadas a `docs/decisions-log.md` e os numeros COLIDEM com decisoes VIVAS ja registradas la sob os MESMOS numeros, com conteudo totalmente diferente: `docs/decisions-log.md:24` DL-0031 (ADR-0031 LGPD DSR identity gate), `:25` DL-0030 (ADR-0030 worker error semantics), `:27` DL-0029 (contingencia RN 639/SIP), `:28` DL-0028 (T2.6 re-scope ANS), `:29` DL-0027 (fechamento PR #18). NAO aplicar este arquivo por cima do decisions-log real — nenhum destes numeros esta livre. Ver a "Nota de numeracao" perto do fim deste arquivo para o detalhe da colisao.
