...
## Current planning — product-first execution guide, 2026-09-10

The user requested renewed planning and a detailed technical guide. [Product-first execution plan](prompts/maezo-product-first-execution-plan.md) now defines the next delivery sequence, superseding the repair-first order below where incompatible while preserving complete scope and independent acceptance requirements. It incorporates BPMN/BPMNDI review, Authorization-first and portal-first delivery, and Tasy through the AMH canonical boundary. Historical execution/paused sections below remain evidence of their respective sessions; saving this guide does not resume implementation or services. Current coordination remains in PLANS/CHECKPOINT.

## Execution resumed — shortest-time strategy, 2026-09-10

The user explicitly requested implementation of `docs/prompts/maezo-completion-strategy-proposal.md`. The graceful pause below is preserved as history, not current scheduling. The full scope and gates remain unchanged. Current queue/source/evidence are in root `PLANS.md` and latest CHECKPOINT/RUNBOOK entries, with cycle evidence in the external evidence store `maezo-completion-evidence/strategy-cycle1-20260910/` (outside this repository). Eight bounded specialists were admitted plus ROOT; final gatekeepers remain unused. Canonical operational limits live in the execution prompt's Throughput controls section. No current source-only result establishes runtime or production acceptance.

## Execution checkpoint — graceful pause, 2026-09-10T01:19:37.373948+00:00

The full scope and acceptance requirements below are unchanged. All currently assigned specialists finished or froze explicitly incomplete WIP and returned; execution is paused at the user's request. Current state, exact source tuples, findings, reviewer continuity, evidence pins and ordered pending work are in `HANDOFF.md` of the external evidence store `maezo-completion-evidence/successor-graceful-handoff-20260910/` (outside this repository) and its `PACKAGES.json`/`final-ready.json`.

Verified source coverage:43/43 task contracts and43/43 read mappings;38DRAFT/five inherited. Initial decision-admission source covers6/43; operational coverage is not established. Global plan completion is UNKNOWN; the75-object inventory is a traceability index, not an equal-weight denominator. Final independent approvals remain0/2. See the measurement report (`maezo-completion-evidence/successor-plan-completion-measurement/REPORT.md`, external evidence store outside this repository).

Combined candidatefc062 is unpublished/unreviewed as a whole. D5fbc repair is source-approved but not propagated. PHI and D7 observer require distinct repairs; controllerD remains WIP with failing checks; HTTP and image-context final verdicts are recorded in the handoff. Complete implementation, source integration, engine qualification, browser/accessibility, ten journeys, deployment, recovery, global ledger and both final gates remain pending to the extents specified there. No source-only result is a production acceptance.


## 1. Source preservation, baseline, and delivery objective

This plan replaces the shorter second plan. It incorporates all substantive requirements from the saved first plan (an external local file, not tracked in this repository), adds the orphan-worktree findings, and preserves its research references, architecture decisions, implementation details, and acceptance criteria.
...
Apply the canonical throughput controls in `docs/prompts/execute-maezo-completion-plan.md` (section of the same name): dependency-ready dispatch, cohesive bounded mandates, exact-scope evidence reuse, grouped candidate validation, delegated preparation, resource-specific serialization and informative diagnosis before expensive retries. Preserve every independent review and final acceptance gate. Keep these procedures in that one canonical location rather than duplicating them here.
