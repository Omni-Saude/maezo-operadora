...
- **D-L [24/09, valores do dev para a Onda 2].** Os pins vão na §4; a ordem de montagem é native-secret → trust digest → catálogo → registro.
  ...
  - **Emenda [24/09, decisão técnica: loader humano recusa '/']:** `KeyDesignation.audience` do `portal-human-material.v1` é `OpaqueRef` (sem `/`). A `audience` de leitura passa a ser `engine-native.maezo-operadora-dev.internal`, sem esquema, igual no portal (pacote humano) e no engine (`read_trust.audience` do native-secret). Assignment e comando: `engine-native.maezo-operadora-dev.internal:assignment` e `engine-native.maezo-operadora-dev.internal:command`. A chave de assignment é `amh-dev-assignment-20260924`.
...
- **D-O [25/09, H7: o plano de atribuição ATIVO, fora do H1–H6].** O BFF só liga o `human` com `portal_assignment_source.state='active'` (`AssignmentRuntime.start` → `_active`). A linha nunca é escrita à mão: nasce por `PostgresStaffAssignmentAdministration.prepare_change` e só vira `active` no `ack_native` de uma publicação aceita por `AssignmentPublication.java`. **1ª geração = só as memberships staff revisadas**, sem policy/binding/designação: `/tasks` e as filas funcionam; claim/release/reassign seguem recusados pelo engine (fica para quando houver policy revisada). Provado no C1 (`run.sh all`, passos `assignment` e `portal-human`: `?queue=team` 200 com a tarefa só para o grupo da DMN, `?queue=mine` 200 vazio para os dois). Passos no dev, **nesta ordem**, todos pelo caminho sancionado:
  1. `python -m tools.staff_materials assignment-plane source-key --key-id amh-dev-assignment-source-20260925 --out <novo>` e `assignment-plane trust --spec <staff-materials-assignment-trust.v1> --human-trust <native-secret>/engine-run/trust.json --human-keys <human-bundle>/public/human-keys-summary.json --source <novo>/public/assignment-source.json --out <novo2>` (a chave de leitura é a `amh-dev-assignment-20260924` do pacote humano; o digest impresso é o `configuration_digest`).
  2. Segredo `engine/native-materials`: nova versão com `engine-run/assignment-trust.json` (o materializador já aceita o nome).
  3. Segredo de linhas (`staff-install/rows`): bloco `assignment` = `{schema:"amh", installation:{schema:"staff-assignment-installation.v1", environment, engine_name, database_incarnation, configuration_digest, deployment_receipt, valid_until}, admin:{login:"portal_assignment_admin_amh", password_secret_arn}}`; `staff_ops.assignment_admin_secret_arn` = `staff-install/assignment-admin`. Rodar a task `staff-rows` (reaplica o `engine-native-install.sql` antes: a instalação agora é SELECT + UPDATE(revision_) para o `cibseven_app`) → linha `MZO_HUMAN_ASSIGNMENT_INSTALLATION` + login da administração + `deploy/sql/portal-assignment-admin-grants.sql`.
