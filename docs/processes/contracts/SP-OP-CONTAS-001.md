...
| External task | `operadora.contas.notify_sla_risk` | consome (worker) | worker `notify_sla_risk`: registra que a ETAPA de alerta de risco de SLA a `coordenacao-contas` rodou (timer nao-interruptivo `BT_AlertaSlaContas`) e retorna `{}` — **NAO afirma `notified` e nao contata canal algum**; nenhuma mensagem sai desta task hoje (GAP-CONTAS-7 / FAB-NOTIFIED-TRIO; antes retornava `notified=True` constante). Informativo e nunca adverso: `UT_AnalistaContas` segue aberta. Ligar o alerta ao canal interno real (`operadora.notifications.internal`, como `recurso`/`lgpd` fazem) segue ABERTO em `docs/review-queue.md` |
