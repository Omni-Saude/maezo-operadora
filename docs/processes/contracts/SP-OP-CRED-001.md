...
| `regiao_saude` | string | nao† | Regiao de saude / area de atuacao do prestador — dado **cadastral/geografico** (Zona Geral, ADR-0006). Compoe o fato `network_changed` que a `network_change_bridge` correlacionaria com SP-OP-ADEQUACAO-001 (`ADEQ-{tenant}-{regiao_saude}-{especialidade}-{ciclo}`) **SE essa ponte existisse** — GAP-XPROC-2 harmonizou o PAYLOAD; o consumidor e um modulo que nao existe em `src/` (DRAFT/verify — PERSP-NETBRIDGE/AF-01, `docs/review-queue.md`). |
...
† **Obrigatoria para a choreografia CRED→ADEQUACAO** (GAP-XPROC-2), **DRAFT/verify quanto ao consumidor** (PERSP-NETBRIDGE/AF-01): o DESENHO e que, SEM `regiao_saude`+`especialidade` no fato `network_changed`, uma `network_change_bridge` nao conseguiria formar a business key de ADEQUACAO e o fato cairia em NO-OP (nao iniciaria adequacao) — mas essa ponte **nao existe** em `src/` hoje (`grep -rn "network_change_bridge" src/` -> 1 hit, docstring em `src/maezo/agents/carolina/graph.py:101`; `find src -name '*network_change*'` -> 0), entao NENHUM fato `network_changed`, com ou sem estes campos, e consumido por producao no momento; o NO-OP e universal, nao condicional a estes campos. Dado cadastral do prestador (nunca PHI de beneficiario, ADR-0006). Semeada no start (agente Carolina / origem da solicitacao).
...
| `tem_plano_substituicao` | boolean | **Flag PLANA de gateway** derivada por `operadora.cred.register_descredenciamento` a partir de `plano_substituicao` (`bool(_norm_str(...))`). Consumida por `GW_Substituicao` (`${tem_plano_substituicao == true}`): `True` ⇒ `End_SubstituicaoRegistrada`, `False`/ausente ⇒ `End_PrestadorDescredenciado`. Idioma de booleano plano do SP-OP-FRAUDE-001 — referenciar `plano_substituicao` diretamente lancaria "Cannot resolve identifier" e travaria a instancia no gateway. **Ambos os terminais sao ADVERSOS** (o prestador foi descredenciado nos dois; o rotulo descreve so o plano de continuidade) |
...
| Kafka | `agents.events.cred.network_changed` | **produz (fato de mudanca de rede); SEM consumidor real hoje (DRAFT/verify — PERSP-NETBRIDGE/AF-01)** | desfecho que altera a rede: `prestador_descredenciado` \| `prestador_credenciado` \| `substituicao_registrada` (payload: `tenant_id`, `prestador_id`, `tipo_prestador`, `regiao_saude`, `especialidade`, `tipo_mudanca`, `data_efeito_iso` — HARMONIZADO com SP-OP-ADEQUACAO-001, GAP-XPROC-2). **DESENHADO** para ser consumido por uma `network_change_bridge` que iniciaria SP-OP-ADEQUACAO-001, correlacionando por `tenant_id`+`regiao_saude`+`especialidade` e derivando `ciclo_avaliacao` do `data_efeito_iso` (trimestre) para a bk `ADEQ-{tenant}-{regiao}-{especialidade}-{ciclo}` — **essa ponte nao existe em `src/`** (`grep -rn "network_change_bridge" src/` -> 1 hit, docstring em `src/maezo/agents/carolina/graph.py:101`; `find src -name '*network_change*'` -> 0); hoje este evento e publicado sem nenhum consumidor de producao |
...
- Contrato de PAYLOAD do fato `network_changed` **harmonizado** com SP-OP-ADEQUACAO-001 (GAP-XPROC-2): o fato carrega `regiao_saude`+`especialidade`+`tipo_mudanca`+`data_efeito_iso`. **NAO harmonizado/NAO construido** (corrigido aqui — PERSP-NETBRIDGE, este contrato afirmava incorretamente que o consumo estava resolvido): o `network_change_bridge` que consumiria esse fato e INICIARIA SP-OP-ADEQUACAO-001 **e um modulo que nao existe em `src/`** (`grep -rn "network_change_bridge" src/` -> 1 hit, docstring em `src/maezo/agents/carolina/graph.py:101`; `find src -name '*network_change*'` -> 0) — achado JA registrado antes deste contrato (`docs/review-queue.md:209,218`), owner-gated em **AF-01** (registro `GAP-REGISTER`, linha `AF-01`, P0). **Pendente de review**: a fonte cadastral de `regiao_saude`/`especialidade` do prestador (base de rede), a periodicidade de `ciclo_avaliacao` (trimestre vs mes — RN 259/566), e a propria decisao AF-01 de construir a ponte ou adotar outro mecanismo — ver docs/review-queue.md.
