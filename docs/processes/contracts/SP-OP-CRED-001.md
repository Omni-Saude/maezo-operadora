...
- Contrato de PAYLOAD do fato `network_changed` **harmonizado** com SP-OP-ADEQUACAO-001 (GAP-XPROC-2): o fato carrega `regiao_saude`+`especialidade`+`tipo_mudanca`+`data_efeito_iso`. **NAO harmonizado/NAO construido** (corrigido aqui — PERSP-NETBRIDGE, este contrato afirmava incorretamente que o consumo estava resolvido): o `network_change_bridge` que consumiria esse fato e INICIARIA SP-OP-ADEQUACAO-001 **e um modulo que nao existe em `src/`** (`find src -type d -name '*bridge*'` -> 0) — achado JA registrado antes deste contrato (`docs/review-queue.md:209,218`), owner-gated em **AF-01** (P0). **Pendente de review**: a fonte cadastral de `regiao_saude`/`especialidade` do prestador (base de rede), a periodicidade de `ciclo_avaliacao` (trimestre vs mes — RN 259/566), e a propria decisao AF-01 de construir a ponte ou adotar outro mecanismo — ver docs/review-queue.md.
