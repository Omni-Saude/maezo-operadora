...
- **Consumidor de retomada (GAP-XHITL-4, implementado):** `src/maezo/platform/integrations/agent_resume.py`, consumer group `maezo-agent-resume`, fila morta `agents.events.process_completed.dlq`. Filtra `resultado=devolvido_agente` (os outros dois desfechos sao confirmados sem efeito); le `notas_resolucao` e `resultado` do HISTORICO do motor pela instancia `_process_instance_id` (operacao catalogada `cibseven.read_historic_variables`, classe `consulta_processo`) e confere chave de negocio, definicao e `resultado` antes de acordar o agente. Um evento que traga qualquer variavel de `phi_vars.PHI_PROCESS_VARS` e' recusado para a fila morta (`resume_phi_in_event`) sem que o conteudo seja lido. Hoje so' a Helena tem porta de retomada; outros `agent_id` sao confirmados com WARNING. O servico nasce desligado: falta a custodia do numero do destinatario (decisao do dono/DPO).
