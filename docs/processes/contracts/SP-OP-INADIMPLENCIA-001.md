...
**Fase:** 3 · **BPMN (shipped, DRAFT):** `spec/processes/bpmn/SP-OP-INADIMPLENCIA-001_Suspensao_Rescisao.bpmn` · **Worker:** `src/maezo/tools/workers/inadimplencia.py` · **Testes:** `tests/integration/processes/test_sp_op_inadimplencia_001.py`, `docs/processes/test-specs/SP-OP-INADIMPLENCIA-001.md` (GAP-INAD-4)
...
| Kafka | `agents.events.inadimplencia.notice_dispatched` | produz | etapa de disparo da notificacao previa publicada; nao comprova entrega nem recebimento |
...
| `ERR_INAD_INVALID_CONTRATO` | worker-guard em `operadora.inadimplencia.handoff_rescisao` (`InadContratoInvalidoError`) | recusa iniciar CANCEL-001 sem `tenant_id` (nao ausente/em branco/`None`) **e** sem `numero_contrato`/`matricula_beneficiario`. **Correcao (D3-01, WP-ADR-0030-COMPLETION):** `Error_InadContratoInvalido` esta declarado no catalogo `<bpmn:error>` do BPMN mas SEM `boundaryEvent`+`errorEventDefinition` o referenciando em lugar nenhum do arquivo (mesmo padrao dead-model de `CONTAS-DEAD-ERROR-CATALOG`) — o worker portanto NAO lanca um `WorkerBpmnError`/bpmn error modelado (faria `check_bpmn_error_allowlist.py` falhar clausula (b), raise nao-provado); e uma falha tecnica (`ValueError`), reportada como `failure(retries=0)` — incidente imediato, nunca reentregue. Se um boundary vier a ser modelado no futuro, a migracao para `WorkerBpmnError` e follow-up separado (decisao de programa, mesma postura de `CONTAS-DEAD-ERROR-CATALOG`). |
...
**CATALOGO VIVO NESTE CONTRATO** — o `config/topic_registry.yaml` citado historicamente nao existe nesta arvore (ADR-0041 §5, DRAFT/verify); a validacao disponivel e a convencao de nomes de `src/maezo/platform/topic_registry.py`. Kafka `agents.events.inadimplencia.*`: `received`, `notice_dispatched`, `sla_breached`, `completed`. External-task `operadora.inadimplencia.*`: `resolve_facts`, `check_prior_notice`, `prepare_dossier`, `register_contract_suspension`, `handoff_rescisao`, `notify_sla_risk` (**NAO** `register_contract_rescission` — DROPPED, GAP-INAD-5, nunca wireado a nenhuma task no BPMN shipped). Message BPMN `msg.inadimplencia.pagamento_recebido`, `msg.inadimplencia.notificacao_ack`, `msg.inadimplencia.info_received`. (`operadora.events.publish` ja registrado.) Process key `SP-OP-INADIMPLENCIA-001` **JA REGISTRADO** em `KNOWN_PROCESS_KEYS` (ADR-0016, `src/maezo/tools/process_allowlist.py`) via PR humano CODEOWNERS.
