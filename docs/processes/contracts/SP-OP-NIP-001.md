...
| External task | `operadora.nip.notify_deadline_risk` | consome (worker) | worker `notify_deadline_risk`: registra que a ETAPA rodou e **NAO afirma mais `deadline_risk_notified`** (FAB-SLA-RISK-NOTIFIED-SLICE4 removeu a chave — o worker so logava, nunca publicou). Hoje a UNICA service task neste topico e `ST_SolicitarInfoNip`; os dois alertas de prazo foram convertidos para `operadora.events.publish` em t3.1. As demais chaves do retorno (ecos dos proprios inputs) ficaram inalteradas — ver o achado ABERTO em `docs/review-queue.md` |
