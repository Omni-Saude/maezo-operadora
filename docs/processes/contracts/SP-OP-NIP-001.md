...
| External task | `operadora.nip.notify_deadline_risk` | consome (worker) | worker `notify_deadline_risk`: registra que a ETAPA rodou e retorna `{}` — **NAO afirma nada** (FAB-SLA-RISK-NOTIFIED-SLICE4 removeu `deadline_risk_notified`; NIP-NOTIFY-DEADLINE-ECHO-KEYS removeu as quatro chaves remanescentes, que eram eco dos proprios inputs). Hoje a UNICA service task neste topico e `ST_SolicitarInfoNip`; os dois alertas de prazo foram convertidos para `operadora.events.publish` em t3.1. `ST_SolicitarInfoNip` nao declara `camunda:inputOutput`, entao o retorno vazio nao escreve nenhuma variavel de processo — fecha o clobber latente contra `grupo_humano` (variavel VIVA em `UT_ElaborarRespostaNip`, GAP-NIP-2) sem mudar nenhum comportamento hoje observavel (prova no motor, ledger `NIP-NOTIFY-DEADLINE-ECHO-KEYS`) |
...
|---|---|---|---|
| `erro_inicio_processo` | **estado do agente gustavo** — NAO e variavel de processo | no `notify_start_failure` do grafo, via o helper unico `maezo.runtime.start_outcome.notify_start_failure` | o agente TENTOU iniciar SP-OP-NIP-001 pelo chokepoint `start_process_idempotent` e o engine recusou (`CibSevenError`). NENHUMA instancia nasceu |
...
gravado a montante (o desfecho de sucesso do fluxo NIP) sobrevivia — o estado do agente AFIRMAVA um fato que nao
