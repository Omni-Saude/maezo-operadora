...
| Kafka | `agents.events.programa.completed` | produz | fim (payload.desfecho = `enrollment_realizado` \| `nao_elegivel` \| `sem_consentimento` \| `interrompido_revogacao` \| `desligamento_clinico_humano` \| `acompanhamento_concluido`) |
...
| External task | `operadora.programa.build_care_plan` | consome (worker→A2A) | monta plano de cuidado / dossie para a User Task clinica (delegacao `care.enroll` a Valentina — **sem decidir alta**) |
...
| External task | `operadora.programa.notify_sla_risk` | consome (worker) | alerta `coordenacao-clinica`/`equipe-cuidado` (timer nao-interruptivo) |
