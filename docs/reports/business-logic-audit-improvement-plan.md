...
| GAP-AUTH-4 | AUTH-001 | high | bl | ADR-0008 `authorization_approval.max_value_brl` never read/enforced by any PEP or worker (specific instance of GAP-XHITL-2). **STRUCTURALLY CLOSED 2026-08-06** — `operadora.auth.validate_auto_criteria` (`ST_ValidateAutoApprovalCriteria`, between `BRT_SlaAnalise` and `BRT_AutoApproval`) computes the ceiling plus the technical/regulatory/contractual criteria and `auth_auto_approval.dmn` v0.2.0 reads ONLY those + `auto_criteria_verificado` (execution fence). Residual is DATA, not structure: D-07 (`max_value_brl: 0`) and the unratified clinical/regulatory/contractual rule sources — see `docs/review-queue.md` and `docs/processes/contracts/SP-OP-AUTH-001.md` | ADR-0008 | m |
...
| GAP-AUTH-3 | AUTH-001 | high | bl | `dut_rol_coverage`/`dut_criteria_*`/`carencia_check` documented as SP-OP-AUTH-001 DMN consumers but no code path (Rafael's graph, workers) invokes them via mcp-dmn. **CLOSED 2026-08-06** — all 5 are now evaluated engine-side by `operadora.auth.validate_auto_criteria` through the ADR-0028 `dmn=` worker seam (NOT a businessRuleTask, so they remain PERMANENT orphans in `orphans-allowlist.yaml`, like the 7 `fraude_scoring` tables), and `docs/review-queue.md`'s false "wired via Rafael" claim was corrected. Being wired does NOT make them deployable: the ratification gate keeps each inert while DRAFT, contributing shadow evidence only | ADR-0012 | l |
