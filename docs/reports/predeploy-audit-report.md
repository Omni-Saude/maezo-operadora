...
> **ADENDO DATADO 2026-09-06 (gap `ADR-PHANTOM-PATH-RESIDUE-NON-ADR`):** the `gateway-policies-path-hard-frozen` and `gateway-policies-path-tenant-overlay` rows below name `src/maezo/policies/autonomy/_hard_frozen.yaml` / `src/maezo/policies/autonomy/tenants-amh.yaml` as the "real path" fix for `gateway.md`'s defects — that "real path" was ALSO wrong: `src/maezo/policies/` has never existed on disk (`.github/CODEOWNERS` documents this since 2026-08-13; confirmed today by `git ls-files src/maezo/policies` -> empty). The genuinely real path, both then and now, is `spec/policies/autonomy/`. `gateway.md` has since been corrected (all 9 sites) to cite `spec/policies/autonomy/`. This table is a historical (2026-07-04) findings ledger and is not rewritten retroactively; this addendum only flags that its own suggested fix was itself phantom.
