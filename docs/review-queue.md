...
| `spec/processes/dmn/{upcoding_complexity_ceiling,unbundling_partial_bundles,phantom_no_diagnosis,phantom_suspicious_prefix,frequency_zscore_threshold,provider_peer_deviation,risk_thresholds}.dmn` (7 portadas byte-fiel — ported, wiring pending T2.7 fase 2 apos T1.4; orphan-allowlisted) | Limiares de indicadores (upcoding/unbundling/phantom/zscore/peer-deviation/risk) — SINTETICOS; `score` e FATO DE ROTEAMENTO, nunca veredito; ver T2.7 PR body para divergencias reportadas (saida `resultado`/`BLOQUEAR` tensiona com invariante L0-hard; entradas nao batem com o worker doador v1) | medico-auditor + auditoria especial + financas | `DRAFT — requires human review before any deploy` |
