...
| `spec/processes/bpmn/SP-OP-AUTH-001_Autorizacao_Previa.bpmn` — `BRT_AutoApproval` (**GAP-AUTH-4**) | Na rota automatica, `dut_atendida`/`dentro_teto_l2`/`rede_credenciada` chegam SEMEADOS NO PAYLOAD DE START e a DMN decide `AUTO_APROVAR` sobre eles sem verificacao. `CeilingResolver`/`within_l2_ceiling` NAO roda antes do `GW_AutoAprovacao` (unico chamador em AUTH = `AnalyzeRequestWorker` em `ST_PrepararDossie`, na perna humana *depois* do gateway) — logo o teto por tenant e **DECORATIVO** nesta rota e **decidir o D-07 NAO fecha isto**. Desde item-9 auth Class-A a rota emite um `numero_autorizacao` TISS real (antes nao emitia nada, apesar de publicar `desfecho=aprovada_automatica`). Remedio = worker deterministico computando os tres fatos ANTES de `BRT_AutoApproval` (precedente: GAP-REEMBOLSO-5). **MITIGACAO PARCIAL (2026-08-05), gap CONTINUA ABERTO:** `IssueAuthorizationWorker` passou a verificar o teto no PONTO DE EMISSAO, **so no canal AUTOMATICO** (`CeilingResolver.within_l2_ceiling`; recusa com `ERR_AUTH_AUTO_CEILING_NOT_AUTHORIZED`, fail-closed em tenant/valor/resolver) — o canal humano (`decisao_auditor == 'APROVAR'` + junta) segue **sem** gate de teto, por design. A DMN continua decidindo sobre fatos semeados no start, e o `valor_estimado_brl` comparado vem do mesmo payload nao verificado. Efeito observavel HOJE (`max_value_brl: 0`): a rota automatica alcanca `End_AprovadaAutomatica` e publica `desfecho=aprovada_automatica` **sem** emitir `numero_autorizacao` — o processo anuncia uma aprovacao que nao emitiu; agora por motivo principiado, logado e auditavel. **A promocao de SP-OP-AUTH-001 de DRAFT para FINAL esta VINCULADA ao fechamento deste gap.** MITIGACAO PARCIAL (item-9 auth Class-A): `IssueAuthorizationWorker` consulta o teto no canal AUTOMATICO antes de emitir, entao com `max_value_brl: 0` nada e emitido automaticamente. **Isto e mitigacao, NAO cerca** (GK-ceiling finding 1): o canal e discriminado por `auto_sanctioned and not human_approved`, nada no BPMN escreve `human_approved`, e nao existe allowlist de VARIAVEIS de start — logo quem consegue forcar a rota automatica pode semear `human_approved: true`/`decisao_auditor: 'APROVAR'` e pular a verificacao. Nao concede nada alem da linha de base anterior (que emitia incondicionalmente na sancao automatica), e discriminar pela variavel activity-local arriscaria bloquear emissao aprovada por HUMANO (negacao de cuidado, o erro pior). So fecham isto: o worker pre-gateway (MZO-040) + estreitar `DEFAULT_ALLOWED_PROCESS_KEYS`. RESIDUO DE SUB-EMISSAO (GK-ceiling finding 4): sob recusa a rota ainda atinge `End_AprovadaAutomatica` e publica `desfecho=aprovada_automatica` SEM numero, sem incidente e sem fallback humano — hoje isso vale para TODA requisicao auto-roteada. Remedio = rotear a recusa para revisao humana (mudanca de BPMN, MZO-040). ADJACENTE (finding 5): `AnalyzeRequestWorker` deriva o mesmo campo capturando so `(TypeError, ValueError)` — `float('inf')` levanta `OverflowError` nao tratado (direcao fail-closed, mas as duas derivacoes divergem no MODO de falha; alinhar em follow-up). | medico auditor + ANS/regulatorio + seguranca | `DRAFT — OPEN GAP, blocks FINAL promotion` |
