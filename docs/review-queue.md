...
Dois gaps de harness/CI fechados nesta sessão por um agente R2 sem engine/broker concedido —
ambos `implemented — unverified`, pendentes de reprodução por um verificador independente.
Nenhum conteúdo clínico ou regulatório aqui; registrados por completude do processo, não por
exigência da tabela principal acima.

| Artefato | O que precisa de revisao humana | Revisor | Status |
|---|---|---|---|
| `tests/integration/platform/test_events_kafka_producer_live.py` (gap `PRODUCER-LIVE-19092-DEFAULT`) | O default de `KAFKA_BOOTSTRAP_SERVERS`/`MAEZO_TEST_DATABASE_URL` foi realinhado ao stack compose real (`localhost:9092`/porta `5433`) e a prontidão de tópico/consumer frio foi portada de `test_notifications_bridge_live_kafka.py` (`_await_topic_ready`/`_await_assigned`/`seek_to_end`). Provado nesta sessão: caminho de skip alto (`1 passed, 6 skipped`, cada skip citando `localhost:9092`), `--collect-only` (7 testes), ruff/mypy/unit-lane/validate-artifacts/xfail-census/gitleaks. **NÃO provado nesta sessão** (engine/broker não concedidos a este agente): a corrida AO VIVO dos 7 testes contra um stack Kafka+Postgres real — é o verificador R1 com o stack isolado que precisa rodá-la e confirmar que a nova prontidão fecha a corrida fria sem enfraquecer nenhuma asserção | verificador R1 (stack isolado) | `implemented — unverified; prova ao vivo pendente` |
| `tests/unit/ci/test_integration_suites_carry_marker.py` (gap `CI-FENCE-TESTS-COUPLING`) | `_Coverage.covered` deixou de aceitar `via_conftest_chain` como alternativa suficiente — agora exige o marcador do próprio módulo, fechando o acoplamento onde o teste 1 só ficava correto por causa do teste 3. Duas mutações reproduzidas e revertidas nesta sessão (não commitadas): apagar o teste 3 (suite continua verde) e, com o teste 3 ainda apagado, remover `pytestmark` de um módulo cujo `conftest.py` pai carrega o marcador (`test_sp_op_adequacao_001.py`, restaurado por byte após a prova) — RED confirmado nos dois testes que checam cobertura própria. Verificador independente deve reproduzir as mesmas duas mutações a partir do commit `e7f041a` | verificador R1 | `implemented — unverified; mutações reproduzidas pelo autor, pendente reprodução independente` |
