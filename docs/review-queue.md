...
| `docs/evidence-ledger.md` — as duas linhas `LEDGER-HASH-RECOMPUTE-CHECK` desta mesma PR | `make check-ledger-hashes` sobre HEAD desta branch reporta `1/2 declared rows verified`, NAO green total: a linha original (`5797aae`, hash sobre 43 testes) ficou STALE BY DESIGN dentro da PROPRIA PR porque o commit seguinte (`036f635`) cresceu o arquivo citado para 44 testes ANTES do merge — processo do autor (linha de ledger adicionada cedo demais), nao defeito do gate; a linha de reconciliacao (`fcac601`, hash sobre 44 testes) verifica GREEN isoladamente. Registrado aqui em vez de escondido; o verificador decide se aceita as duas linhas como estao (demonstracao real do caso stale-by-design) ou pede uma unica linha final | verificador R2 designado | `disclosed — nao e regressao do gate` |
