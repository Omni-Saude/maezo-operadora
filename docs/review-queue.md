...
| `src/maezo/agents/helena/prompts.py::SINTOMA_EM_PALAVRAS` (mapa `sintoma_codigo` -> palavras que a Helena DIZ ao beneficiario na frase de confirmacao da reavaliacao, F5) | **TRADUCAO CLINICA voltada ao paciente.** Hoje todo o mapa e' `None` (a Helena usa a frase generica "o mesmo sintoma que voce contou") EXCETO `febre`, que e' a palavra que a propria pessoa usa. Cada entrada nao-`None` e' uma afirmacao de EQUIVALENCIA (`dispneia` -> "falta de ar", `deficit_neurologico` -> ?) que pertence a um revisor clinico, nao a quem escreve o grafo; e para os cinco codigos de saude mental ler o codigo de volta devolveria o diagnostico a pessoa (`ideacao_suicida`), que e' dano e nao transparencia. O que precisa de revisao: (a) `febre` esta' correto como unica entrada com texto? (b) quais outros codigos podem ser ditos em portugues leigo, e com quais palavras exatas? | medico auditor | `DRAFT — ampliar a lista exige revisao humana; o default `None` (frase generica) e' seguro e nao bloqueia deploy` |
| `src/maezo/agents/helena/graph.py::_TETO_DE_IDADE` — a AUSENCIA de teto para `idade_gestacional_semanas` | **Limite demografico declarado para duas fronteiras de idade (memoria clinica e extracao do turno), e uma lacuna DELIBERADA na terceira.** Os dois numeros escritos nao sao conteudo clinico: 120 anos e' limite demografico (recorde documentado: 122) e 288 meses e' 24 anos, o mesmo limiar que a frase de confirmacao ja' usa; nenhum decide conduta, decidem se um valor E' uma idade. `idade_gestacional_semanas` ficou SEM teto porque qual semana deixa de ser uma gestacao possivel e' julgamento clinico (pos-termo existe). O que precisa de revisao: existe um limite superior de semanas acima do qual o valor deve ser tratado como extracao torta (-> `invalid_age` -> escalonamento por falha tecnica), e qual? | medico auditor | `DRAFT — sem teto hoje; um valor absurdo de semanas atravessa as duas fronteiras e chega a tabela gestante` |
...
| `deploy/helm/maezo-tenant/templates/deployment-bridge-netchange.yaml` | R-198 decidiu principals de auditoria DISTINTOS por ponte. O chart separa somente identidades Kubernetes: a ponte de rede renderiza a ServiceAccount própria `maezo-network-change-bridge`, enquanto notifications-bridge usa `maezo-tenant.serviceAccountName`; ambas mantêm `automountServiceAccountToken: false`. ServiceAccount/IRSA é identidade de nuvem, não prova de identidade de aplicação na trilha ADR-0007/ADR-0033. O módulo `network_change_bridge` continua ausente e `networkChangeBridge.enabled=false`; o Deployment fica DORMANT e não demonstra daemon nem principal de auditoria em execução | arquitetura + segurança | `PARCIAL 2026-09-08 — decisão R-198 preservada; segregação K8s entregue. NETBRIDGE-SERVICE-PRINCIPAL-IDENTITY continua pendente antes de habilitar o daemon: emitir e verificar principal de aplicação distinto no chokepoint de audit; ADR-0033 segue Proposed/não vinculante` |
...
| `docs/adr/0040-...md` OQ-R2 | Arredondamento em `DEFERIR_PARCIAL` — a invariante `valor_deferido_brl + valor_glosa_mantido_brl == valor_glosado_brl` e imposta em centavos-inteiros com igualdade exata. Se a operacao usa tolerancia ou arredondamento contratual, o guard do worker precisa refletir isso; ate la o guard recusa a soma que nao fecha | financas + auditoria de contas | `PERGUNTA FECHADA — decisao do dono R-155 (2026-09-04), aplicada 2026-09-06`: a igualdade exata em centavos-inteiros passa a ser invariante PERMANENTE do guard de `registrar_indeferimento`, e o sign-off de SP-OP-RECURSO-001 deixa de esperar por ela. **NAO e ratificacao de financas**: a decisao so APERTA (fixa o comportamento ja vivo); qualquer AFROUXAMENTO — tolerancia ou regra de arredondamento — continua humano e entra como regra nova assinada por financas, em PR proprio. Fixada por teste em `tests/unit/tools/workers/test_recurso.py` (seccao R-155). Divergencia conhecida: `docs/adr/0040-...md` (CODEOWNED) ainda lista OQ-R2 como aberta — reconciliacao do texto da ADR pendente do dono |
...
| `docs/processes/contracts/SP-OP-PAGTO-001.md` (GAP-PAGTO-9, hitPolicy `pagto_sla`; CONTRACT-HITPOLICY-DRIFT) | **RESOLVIDO 2026-09-05 (R4-DOCS-W4).** Cabecalho corrigido de `UNIQUE` para `FIRST` (`spec/processes/dmn/pagto_sla.dmn:26` ja era `FIRST`) — texto apenas, DMN nunca tocada. A restricao de propriedade PR-3/PR-4 desta linha original nao se aplicava mais nesta rodada (dossie CONTRACT-HITPOLICY-DRIFT do round-5, sem essa ressalva). Entrada removida de `_HIT_POLICY_DRIFT_BASELINE` (agora `frozenset()` vazio); `test_no_new_contract_claims_collect_or_unique_for_a_dmn_actually_deployed_as_first` PASSED confirma ausencia de regressao e a propria asserção de baseline resolvido | docs-finalizer (R2) | `RESOLVIDO — texto do contrato corrigido para FIRST nesta PR (r5/r4-docs-w4)` |
...
| `spec/agents/*/agent.yaml` — bloco `model:` | **Tier `batch` nao e' declarado por nenhum agente.** ADR-0009 §2 nomeia tres tiers ("lote -> batch tier"); os 10 agentes + `_template` declaram apenas `task_default` e `reasoning`. O vocabulario aceita `batch` (`inference.MODEL_TIERS`), mas NENHUM `agent.yaml` foi editado para introduzi-lo: qual trabalho pode ser diferido para lote e' decisao de produto/assistencial, nao detalhe de implementacao, e inventar a declaracao recriaria a config-morta que AF-12 acabou de remover | PO + gestao assistencial | `RESOLVIDO 2026-09-06 (BATCH-TIER-CARGA-PRODUTO / R-195, OWNER-DECISIONS-REGISTER, APROVADO-APOS-REVISAO-HUMANA, decisor PO/produto + gestao assistencial, resposta_sugerida verbatim: "B — mantem nenhum agente no tier batch ate o produto nomear uma carga concreta, porque declarar tier sem carga e capacidade especulativa") — DECISAO DE NAO FAZER NADA, registrada: nenhum agent.yaml muda de tier agora; a condicao de reabertura e uma carga concreta nomeada pelo produto. Nenhuma edicao em spec/agents/*/agent.yaml por esta linha. O gate de eval golden de M-49/TIER-MODEL-MAP (§5 q64) continua exigido antes de qualquer tier apontar para modelo mais barato, independente desta linha` |
...
| `sintoma_reconhecido` (`spec/processes/dmn/triage_sufficiency.dmn:58`, `dmn_input_expression`) | BOOLEANO da tabela de suficiencia da Frente 2.2, derivado em `agents/helena/graph.py::classify` como `sintoma_codigo is not None`. A heuristica de forma casou pelo radical `sintoma`, o mesmo de `sintoma_codigo`, que JA' e' shape-suspect listado. Nenhuma outra superficie declara este nome. **Recomendacao da engenharia: NAO listar**, e a razao e' o DESENHO da tabela, nao um juizo sobre o nome — ela e' um SINALIZADOR DE PRESENCA: responde 'a mensagem casou um dos 25 codigos da allowlist?' com verdadeiro/falso, e nao consegue carregar QUAL codigo, qual sintoma, nem palavra nenhuma do beneficiario. A tabela foi construida assim de proposito: ela decide se o DADO BASTA para responder com honestidade, nunca o que o dado diz, entao as seis entradas sao dois vocabularios fechados (`intent`, `population`), tres booleanos e um contador de rodadas. Nada de texto livre a alcanca. **Contraevidencia que o DPO precisa pesar, na forca real que ela tem:** um booleano DERIVADO de PHI pode ser PHI quando a inferencia e' forte — `true` aqui significa 'esta pessoa descreveu algo que o vocabulario de triagem conhece', o que num canal de saude ja e' uma inferencia clinica sobre uma conversa identificada. O argumento nao e' frivolo; o que pesa contra ele e' que a MESMA inferencia ja viaja, com muito mais resolucao, no proprio `sintoma_codigo`, que esta listado. Proteger o sinalizador enquanto o codigo viaja ao lado nao protege nada. Se a leitura do DPO for outra, o conserto barato e' na FONTE (parar de por `sintoma_codigo` em variavel de processo), nao neste derivado. **A TABELA NAO ESTA IMPLANTADA** e o no' de coleta segue desligado (`coleta_enabled=False`), entao a pergunta esta sendo registrada ANTES de o artefato existir no motor — que e' a ordem que esta cerca existe para impor | DPO + medico auditor | `DRAFT/verify (DPO) — pergunta aberta` |
...
| `src/maezo/tools/workers/harness.py` `_SAFE_DECISION_BASIS_KEYS` continha `"notice_sent"` (AUTH-CONTRACT-TRANSMIT-PROSE) | **RESOLVIDO 2026-09-05 (R4-DOCS-W4).** A lista e de CHAVES de saida do worker, nao de valores (`build_decision_basis` faz `if key in out_vars`). Confirmado de novo nesta rodada: `grep -rn '"notice_sent"' src/maezo/` nao encontra nenhum worker de producao devolvendo essa chave — so a docstring historica de `auth.py:1427-1431` e o fixture `tests/unit/gateway/test_action_execution_gateway.py` (que passou a usar `admissivel`, chave ja allowlisted, mesmo comportamento exercitado). Chave removida de `_SAFE_DECISION_BASIS_KEYS` com comentario citando a evidencia; 2 testes novos em `test_harness.py` pinam a remocao e provam que `build_decision_basis` continua rejeitando chaves fora do allowlist (mutacao: reintroduzir a chave derruba os 2 testes) | docs-finalizer (R2) | `resolvido 2026-09-05 (AUTH-CONTRACT-TRANSMIT-PROSE, r5/r4-docs-w4)` |
| prosa dos codigos de erro em `docs/processes/contracts/SP-OP-AUTH-001.md` (AUTH-CONTRACT-TRANSMIT-PROSE) | **RESOLVIDO 2026-09-05 (R4-DOCS-W4).** As linhas de `ERR_AUTH_DENIAL_INCOMPLETE` e `ERR_DENIAL_NOT_HUMAN` diziam "recusa TRANSMITIR uma NEGAR". Reescritas para "recusa compor o registro de uma NEGAR" — confirmado que nada e transmitido nesta etapa em circunstancia alguma: o que os guards impedem e a COMPOSICAO do registro e o avanco do token ate `End_NegadaAuditor`. Verificado por leitura direta de `docs/processes/contracts/SP-OP-AUTH-001.md:228-229`: a string "recusa TRANSMITIR" nao existe mais no arquivo | dono do processo AUTH + regulatorio | `resolvido 2026-09-05 (AUTH-CONTRACT-TRANSMIT-PROSE, r5/r4-docs-w4)` |
...
| `HELM-ENV-PASSTHROUGH-UNREAD` (follow-up gap, F2) — `ENVIRONMENT` (declarado via `agents[].env`, passthrough livre de operador em `values-amh.yaml`, renderizado por `{{- range $k, $v := $agent.env }}` em `deployment-agent-runtime.yaml`) nao tem leitor em `src/` sob nenhuma grafia | Decidir se `ENVIRONMENT` deveria ser lido por algo, removido do values, ou permanece um passthrough sem uso hoje. NAO e' a mesma classe de defeito de DU-02 (nao ha renomeio a fazer) — allowlisted como `DEFERRED_UNRECONCILED_DECLARED`, nao corrigido especulativamente neste PR | `@rodaquino-OMNI` ou dono do agent-runtime | `RESOLVIDO 2026-09-06 (SMALL-DECISIONS, R2 sonnet) — REMOVIDO: chave \`ENVIRONMENT: "staging"/"prod"\` apagada do bloco \`env:\` de todos os 5 agentes em \`values-staging.yaml\` e \`values-amh.yaml\` (\`env: {}\` no lugar, mesmo default da base \`values.yaml\`); confirmado 0 leitores em \`src/\` (busca por \`\\bENVIRONMENT\\b\` — os unicos hits sao o enum \`CredentialSource.ENVIRONMENT\` de \`capabilities.py\`, que nao le a var, e um comentario). Entrada correspondente removida de \`DEFERRED_UNRECONCILED_DECLARED\` (scripts/ci/check_chart_env_reconciliation.py) — a tabela fica vazia (ambas as entradas historicas fechadas na mesma leva). \`uv run python scripts/ci/check_chart_env_reconciliation.py\` -> "92 declared... 0 deferred... OK"` |
| `HELM-ENV-AUDIT-DIR-DEAD` (follow-up gap, F2) — `AUDIT_DIR` (declarado em `deployment-bridge.yaml` para `notifications-bridge`, valor `/tmp/maezo-audit`) nao tem campo correspondente em `NotificationsBridgeSettings`; o audit trail real do bridge e' `PostgresAuditSink`, nao um diretorio | Decidir se `AUDIT_DIR` deve ser removido do template ou se um uso futuro esta planejado. Nome vestigial, sem leitor sob nenhuma grafia — allowlisted como `DEFERRED_UNRECONCILED_DECLARED` | `@rodaquino-OMNI` ou dono do notifications-bridge | `RESOLVIDO 2026-09-06 (SMALL-DECISIONS, R2 sonnet) — REMOVIDO: bloco \`- name: AUDIT_DIR / value: "/tmp/maezo-audit"\` apagado dos TRES templates que o declaravam (\`deployment-bridge.yaml\`, \`deployment-bridge-consent-revocation.yaml\`, \`deployment-bridge-netchange.yaml\` — o achado original citava so' o primeiro; os outros dois pares sibling carregavam o mesmo bloco morto). O volumeMount/volume \`/tmp\` de cada template e' generico (readOnlyRootFilesystem scratch dir, usado por quase todo Deployment do chart) e foi preservado. Entrada correspondente removida de \`DEFERRED_UNRECONCILED_DECLARED\`. Mutacao provada: re-adicionar o bloco AUDIT_DIR a deployment-bridge.yaml -> 4 FAILED em tests/unit/ci/test_check_chart_env_reconciliation.py (a fence volta a marcar DECLARED BUT NEVER READ, porque a entrada deferred nao existe mais); revertido -> 42 passed` |
...
| `src/maezo/tools/workers/lgpd.py::ExecuteErasureWorker.execute` — egresso CRU de `fundamentacao_legal` | O worker devolve `"fundamentacao_legal": fundamentacao` cru num dict de saida e o escreve numa linha structlog. Hoje e **latente**: o worker esta registrado no topico ORFAO `operadora.lgpd.execute_erasure`, que o BPMN nao declara. **O PR R-181** (colapso dos tres `Execute*Worker` no topico modelado `operadora.lgpd.execute_request`) **torna essa aresta viva** — logo R-181 nao e um PR de nomenclatura e precisa levar junto o tratamento desse campo. Nao corrigido aqui (mudanca de `src/`, fora do lote de rascunhos) | plataforma + DPO | `RESOLVIDO 2026-09-06 (LGPD-EXECUTE-ERASURE-RAW-FUNDAMENTACAO, no MESMO PR de R-181 como a nota exigia): o worker que devolvia o campo cru foi APAGADO no colapso O2-O4; o worker unico lgpd.py::ExecuteRequestWorker NUNCA vincula o texto de fundamentacao_legal a uma variavel local — dele sai no maximo a PRESENCA (tem_fundamentacao: bool), o mesmo idioma de make_send_response_handler, e detalhes_requisicao nao e sequer lido. Correcao ESTRUTURAL, nao redacao: as duas rotas sugeridas (redact_phi_vars / PHI_FREE_TEXT_VARS) sao BLOQUEADAS por pins vivos — test_dpo_drafts_citations.py pina len(PHI_PROCESS_VARS)==8 e pina os DOIS nomes FORA dos dois conjuntos (o §2.4 do runbook do DPO depende disso), e pina os call sites de redact_phi_vars em exatamente auth.py + transport.py; nenhum pin do corpus PHI (CORPUS_DELTA_LOG) foi movido. ABERTO o que sempre foi do DPO: a DISPOSICAO dos dois nomes — eles seguem fora dos dois controles ancorados em nome, entao o valor digitado continua cru nas variaveis de processo do motor e em qualquer novo call site. Cercas: test_lgpd_erasure.py::test_execute_request_nunca_emite_texto_livre_phi (comportamento) e ::test_execute_request_nao_le_o_texto_de_fundamentacao_em_lugar_nenhum (fonte)` |
...
## Lacuna pre-existente na guarda RAF-02, encontrada por VERIFY-TRAIN2 §Delta (2026-09-05)

Nenhuma linha acima foi editada. A entrada abaixo diverge um caso encontrado no reparo do trem
`r5/train-2` (F4, `fbedc5e` -> `§Delta-F1`): o docstring de `record_agent_error` generalizava "cada
handler A2A levanta um StartProcessFailedError tipado (RAF-02)" quando isso e' verdade so' para
QUATRO dos oito handlers. O defeito abaixo e' PRE-EXISTENTE em `origin/main` `75ed74fb` e esta'
FORA DE ESCOPO deste PR — divulgado aqui, nao corrigido.
...
## R-181 / LGPD-WORKER-DRIFT — colapso O2-O4 no tópico modelado (EXECUTADO 2026-09-06)

| Achado | O que foi feito | Dono | Status |
|---|---|---|---|
| `register_lgpd_workers` registrava TRÊS workers em tópicos que o BPMN não declara (`operadora.lgpd.execute_export`/`execute_rectification`/`execute_erasure`, linhas O2-O4 de `docs/compliance/lgpd-topic-reconciliation.md`) enquanto o tópico MODELADO `operadora.lgpd.execute_request` (`ST_ExecutarRequisicao`, `bpmn:271`) ficava SEM worker (linha T4). Gap `LGPD-WORKER-DRIFT` (que ainda dizia **quatro** workers — `publish_completed` já havia saído com R-H em `abb9d60`, então eram **três**) | Colapso autorizado pela decisão do dono **R-181** (classe A): as três classes foram APAGADAS e substituídas por `lgpd.py::ExecuteRequestWorker`, que despacha pelo `tipo_requisicao` declarado (contrato + `bpmn:documentation`, seis valores, cerca derivada do contrato) e **recusa em todo caminho** com `WorkerFailureError(retries_left=0)`. Duas asserções antigas foram INVERTIDAS de propósito porque pinavam sucesso FABRICADO (`export_compiled` com `package_ref` de `uuid4()`; `rectification_completed`) — latente enquanto os tópicos eram órfãos, VIVO no tópico modelado. Drift provado FECHADO pela árvore (parse do BPMN × introspecção do harness), nos dois sentidos | plataforma (topologia) + DPO (habilitação) | `RESOLVIDO 2026-09-06 quanto à TOPOLOGIA. ABERTO, e sempre foi do DPO: (a) a habilitação da execução REAL (F-2 + matriz AF-07) — hoje NENHUM caminho executa; (b) o worker de operadora.lgpd.compile_data_package (#55 R-C), que precisa da matriz de bases legais/retenção; (c) se um bpmn:boundaryEvent modelado deve ser adicionado a ST_ExecutarRequisicao (rota (i) de R-D, mudança de spec). Contrato segue DRAFT v0.1.0, nenhum signoff tocado` |
| **Consequência observável em produção** que o colapso introduz: com o worker registrado e recusando, uma DSR aprovada como `EXECUTAR_E_ENVIAR` levanta incidente no motor e **não conclui** (antes, o tópico modelado não tinha worker e a tarefa ficava pendente para sempre) | Deliberado: `Flow_Executar_Enviar` não tem `conditionExpression`, então qualquer `complete` — inclusive um dict `blocked_by_guard`, que era o comportamento dos três workers aposentados — avançaria o token para `ST_EnviarResposta` → `event_desfecho=atendida`, afirmando ao titular uma execução que não houve. O incidente é o único desfecho que SEGURA o token | DPO + dono do processo LGPD/DSR | `DECLARADO 2026-09-06 — precisa ser visto na assinatura; documentado em docs/sme-dispatch/dpo/DSR-PROCEDURE-DRAFT.md §8.2 e no PACKAGE.md (atualização sob a pergunta R-D)` |
| **Achado adjacente, NÃO corrigido aqui:** `human_approved` — o sinal em que os guards L0 dos três workers aposentados se apoiavam — **não existe no BPMN nem no contrato** (`grep -n 'human_approved'` nos dois = 0 ocorrências). Nenhuma instância real o semeia; a decisão humana RATIFICADA do processo é `decisao_dsr`, escrita por `UT_RevisaoDpo` e guardada pelo engine em `Flow_GWDec_Executar` | Guard mantido (defesa em profundidade, matriz T3.1 de 8 vetores intacta), mas NUNCA relatado como causa primária: `_RECUSA_PREFIXO` lidera toda mensagem de incidente com o fato invariante (nada implementado) antes de nomear o guard — senão o diagnóstico de produção culparia o revisor por uma lacuna de implementação. **Enquanto nada executa, isso é inofensivo**; vira decisão real no PR de habilitação: ou `human_approved` passa a ser semeado pelo modelo, ou o guard L0 passa a ler `decisao_dsr` | DPO + dono do processo LGPD/DSR | `ABERTO — pré-condição do PR de HABILITAÇÃO (F-2 + AF-07), não da topologia. Candidato a linha do GAP-REGISTER` |
...
de escrever cada golden `EVL-<AGENTE>-JOURNEY-01.json` (ver `docs/evidence-ledger.md` linha `R6-JOURNEY-EVAL-EXPANSION`
(alias do finding `11.5`) desta mesma data para a lista completa de arquivos).
...
`main()` falha fechado se o arquivo nao existir, mesma postura de `--adr-dir`. O finding AF-06 fecha aqui; a evidencia nova usa o id lider unico
`R6-TOOLING-COUNTS-FENCE` (alias `AF-06`).

## SUPERFICIE-HUMANA-OPERADOR — decisão "Cockpit + testchannel demo" registrada (R-031, 2026-09-04)

| Artefato | O que foi decidido | Revisor | Status |
|---|---|---|---|
| `src/maezo/platform/testchannel/README.md` (NOVO) + `src/maezo/platform/testchannel/paginas/{exemplo,autorizacao}.html` (gap `9.1`) | O dono decidiu (`OWNER-DECISIONS-REGISTER` R-031, Bold-Decision Review v2 CEO 2026-09-04, `APROVADO-APOS-REVISÃO-HUMANA`), citado verbatim: *"Manter o SIM (Cockpit + `testchannel` declarado demo sem autenticação própria) e DELETAR a espera em vez de datá-la: a linha de `docs/review-queue.md` sai de DRAFT e a nota em `src/maezo/platform/testchannel/README.md` entra no MESMO PR que registra esta resposta do registro — sem teto de calendário, porque não sobra espera a limitar —, e `antes do 1º operador real` deixa de ser âncora e vira mera nota de revisão."* A superfície do operador para as jornadas com User Task é o **Cockpit do CIB Seven** (opção A do memo M-19); o `testchannel` é formalmente **demo sem autenticação própria** (opção D) — `autorizacao.html` (535 linhas) é materialmente uma página de dossiê/decisão para `SP-OP-AUTH-001` com dados reais, mas SEM controle de acesso por papel nem trilha de auditoria de decisão vinculada a usuário nomeado; só está atrás do Cloudflare Access, que autentica a pessoa, não o papel. Nenhuma espera datada permanece: `prazo_ancorado` original (`antes do 1º operador real`) vira mera nota de revisão, não âncora | PO / produto (dono, ato de sign-off deste registro — não aprovação de agente) | `RESOLVIDO 2026-09-06 — decisão A+D registrada via este PR; desbloqueia WP-SUPERFICIE-HUMANA (11.1, 9.2, 9.6, 10.1, 10.2, 11.7), condicionado ao mapa de personas de R-032 (11.1, M-20)` |

## FILA-T-E-LOTE-UNICO — inadimplencia + cred + cancel registrados como um so lote de habilitacao (R-075, 2026-09-04)

| Artefato | O que foi decidido | Revisor | Status |
|---|---|---|---|
| `scripts/ci/check_bpmn_error_allowlist.py::TE_ENABLED_CODES` (CODEOWNED, `/scripts/ci/`) + `src/maezo/runtime/worker_runtime/service.py::_TE_ENABLED_CODES` (espelho de producao) | O dono decidiu (`OWNER-DECISIONS-REGISTER` R-075, dono engenharia/infra, `APROVADO-APOS-REVISÃO-HUMANA`), citado verbatim: *"agrupar depois — manter `ERR_CONTRACT_SUSPENSION_NOT_HUMAN` T-E-deferred e habilitar no mesmo lote de `cred`/`cancel`."* Os **4 codigos T-E-deferred hoje** (`is_te_gated`, medido via `check_bpmn_error_allowlist.py::run_gate`): `ERR_CONTRACT_SUSPENSION_NOT_HUMAN` (inadimplencia), `ERR_CANCEL_MANTER_NOT_HUMAN` (cancel), `ERR_CRED_DENIAL_NOT_HUMAN` + `ERR_DECRED_NOT_HUMAN` (cred) — formam UM lote unico de habilitacao, nao 3 decisoes pontuais. **Gatilho de habilitacao:** ato explicito e unico do dono (engenharia/infra) que move os 4 codigos de `te_deferred` para `TE_ENABLED_CODES` (gate) + `_TE_ENABLED_CODES` (service.py) NO MESMO PR, auditavel de uma vez — nao ha condicao automatica/data; a justificativa registrada (R-075) e evitar decisoes pontuais boundary-a-boundary, cada uma abrindo uma janela de risco propria. **Ate la:** producao segue tratando cada falha por incidente manual (postura fail-closed preservada, `PermissionError` auditado) — nenhuma mudanca em `scripts/ci/check_bpmn_error_allowlist.py` nem no allowlist de producao nesta linha. Dependencia: mesma leitura de ADR-0030 da pergunta 97 (M-71); informa R-081 (familia INADIMPLENCIA segue T-E-deferred) | dono (engenharia/infra) | `REGISTRADO 2026-09-06 — fila T-E declarada como lote unico via este PR; habilitacao real fica ABERTA, ato futuro explicito do dono` |

## IDEMP-GENERATION-RESIDUAL — residuo de geracoes >=2 aceito e divulgado, ancorado no outbox MZO-060 (R-197, 2026-09-04)
