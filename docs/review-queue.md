...
`docs/sme-dispatch/medico-auditor/PACKAGE.md` (both 2026-07-24). Both artifacts are already
...
| `spec/processes/dmn/programa_routing.dmn` (`risco_estratificado` input) + `operadora.programa.stratify_risk`/`proactive_contact` (unbuilt, `programa.py:222-223`) | Actual clinical stratification criteria for `risco_estratificado` (prescribed nowhere today) + ratification of the DMN's fail-safe-to-`ANALISE_HUMANA` default; `proactive_contact` channel/consent constraints (joint DPO) — see `docs/sme-dispatch/medico-auditor/PACKAGE.md` and `docs/sme-dispatch/dpo/PACKAGE.md` SP-OP-PROGRAMA-001 addenda | médico-auditor (+ DPO on `proactive_contact`) | `DRAFT — requires human review before any deploy` |
