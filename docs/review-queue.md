...
| Artefato | O que precisa de revisao humana | Revisor | Status |
|---|---|---|---|
| `scripts/ci/check_evidence_ledger_hashes.py` + `Makefile:check-ledger-hashes` (CODEOWNED: `scripts/ci/`, `Makefile`) | Confirmar a decisao de design acima (retirada da coluna `[ NN%]` antes de ordenar) contra o hash real de `EVAL-REPLAY-EXHAUSTION-SWALLOWED` quando a branch `fix/eval-replay-exhaustion-unswallowable` mergear nesta base — hoje so validado por precedente indireto (linha `t1.1 (defect fix)`/commit `35b8e3e`, que documenta a MESMA retirada por um motivo independente) e pela suite de 43 testes unitarios (incl. um fixture real que prova hash-com-marcador != hash-sem-marcador) | dono do repo (CODEOWNED) + verificador R2 designado | `implemented — unverified` |
