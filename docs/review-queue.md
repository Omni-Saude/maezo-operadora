...
tres raizes; `maezo_tool_calls_total`/`maezo_agent_errors_total` passaram a ser incrementados;
os topicos servidos por handler cru voltaram a emitir as metricas M11; e `task_kind` atravessa
`InferenceProvider.generate()` a partir do `model:` do `agent.yaml`.
...
| `src/maezo/platform/observability.py` — `setup_observability` (FORMATO DE LOG) | **Mudanca de formato de log em producao, ja' aplicada e divulgada aqui.** Ligar as raizes troca a configuracao DEFAULT do structlog pelo `ConsoleRenderer` + `PrintLoggerFactory` do modulo. Isso era o motivo declarado (`_build_key_scrubber`, pre-AF-13) para o wiring ser uma mudanca separada: nao e' byte-identico. Nao ha' decisao pendente de conteudo — o item existe para que SRE/on-call saiba que o formato dos logs dos tres daemons muda neste deploy, e para que a decisao de trocar por um renderer JSON (se e' isso que o pipeline de logs espera) seja tomada com os olhos abertos | SRE/observabilidade | `APLICADO — divulgado, nao revertido; renderer JSON e' decisao aberta` |
