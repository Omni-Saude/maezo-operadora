...
Contexto que amarra os tres: enquanto (1) nao acontece, o gate de promocao a producao construido em
`.github/workflows/cd.yml` vale apenas dentro do grafo do proprio workflow de CD; enquanto (3) nao
acontece, `require_code_owner_review` continua inerte e a fila `owner-review` inteira tem um unico
revisor possivel, que e tambem o autor dos PRs.

| Artefato | O que precisa de decisao/acao humana | Revisor | Status |
|---|---|---|---|
| ruleset `main-protection` (GitHub, id 20775655) — required status checks | **(1)** Tornar `require-production-approval` (job novo em `.github/workflows/cd.yml`, R-021) um required status check, ANTES de qualquer PR que defina `AWS_ENABLED=true`. Medido 2026-09-04 (`gh api repos/Omni-Saude/maezo-operadora/rulesets/20775655`): enforcement `active`, `required_status_checks` = exatamente 4 contextos — `lint / type / unit`, `gitleaks / secrets`, `validate-artifacts`, `release-capability-floor (audit §5)`. O job novo nao esta entre eles. O agente construiu o gate; exigi-lo e ato do dono. | dono (infra + plano GitHub) | `ABERTO — ato do dono; gate construido, ainda nao exigido` |
