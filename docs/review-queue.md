...
| `deploy/helm/maezo-tenant/templates/deployment-bridge-netchange.yaml` | Daemon novo (sibling do notifications-bridge; MESMA identidade de principal de servico `notifications-bridge` — allowlist minima start_process). Confirmar se as duas pontes devem compartilhar a identidade ou ter principals distintos na auditoria | arquitetura + seguranca | `RESOLVIDO 2026-09-06 (NETBRIDGE-SERVICE-PRINCIPAL-IDENTITY / R-198, OWNER-DECISIONS-REGISTER, APROVADO-APOS-REVISAO-HUMANA, decisor dono(arquitetura)+seguranca, resposta_sugerida verbatim: "A — principals DISTINTOS por ponte, que e' o menor privilegio e o que torna a trilha de auditoria atribuivel") — network-change-bridge ganhou ServiceAccount PROPRIA (\`maezo-network-change-bridge\`, serviceaccount.yaml + helper \`maezo-tenant.networkChangeBridgeServiceAccountName\` em _helpers.tpl), gated pelo mesmo \`networkChangeBridge.enabled\`; notifications-bridge permanece na SA compartilhada \`maezo-tenant.serviceAccountName\`. \`automountServiceAccountToken: false\` preservado nas duas. helm template confirma nomes DISTINTOS (\`maezo-network-change-bridge\` vs \`maezo-agent\`); pinado por tests/unit/deploy/test_bridge_service_account_identity.py` |
