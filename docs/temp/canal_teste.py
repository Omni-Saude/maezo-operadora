"""Canal de teste local — Etapa 4 do Plano de Teste (formulário de lançamento + evidência).

Uso:  uv run python docs/temp/canal_teste.py   →  abre http://localhost:8500

Serve uma página única e faz proxy de /engine/* para o CIB Seven local
(http://127.0.0.1:8080/engine-rest/*), evitando CORS. Nada além da stdlib.
Ferramenta de TESTE local: nunca implantar; o motor local não tem auth.
"""
...
ENGINE = "http://127.0.0.1:8080/engine-rest"
PORT = 8500
...
class Handler(BaseHTTPRequestHandler):
    def _proxy(self) -> None:
        ...
        req = urllib.request.Request(
            ENGINE + path, data=body, method=self.command,
...
if __name__ == "__main__":
    print(f"Canal de teste: http://localhost:{PORT}  (motor: {ENGINE})")
