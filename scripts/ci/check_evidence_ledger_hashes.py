...
Every recipe subprocess receives a minimal environment with no inherited credentials,
`PYTEST_ADDOPTS`, or plugin autoload. Historical execution also fences every imported `maezo`
module inside its owned archive and refuses a source lock different from the candidate lock.
...
def recipe_environment(*, python_path: str | None = None) -> dict[str, str]:
    """Minimal deterministic pytest environment: no credentials or ambient pytest options."""
...
def _module_paths():
    ...
    for name, module in sorted(sys.modules.items()):
        if name != "maezo" and not name.startswith("maezo."):
            continue
...
def _escaped(paths):
    return [path for path in paths if not path.is_relative_to(_ROOT)]


def pytest_sessionstart(session):
    importlib.import_module("maezo")
    escaped = _escaped(_module_paths())
    if escaped:
...
def _capture_historical_recipe(repo_root: Path, edge: SupersessionEdge, python_exe: str) -> PytestCapture:
    ...
    with tempfile.TemporaryDirectory(prefix="maezo-ledger-history-") as tmp:
        ...
        imported_paths = [Path(line) for line in marker.read_text(encoding="utf-8").splitlines() if line]
        if not imported_paths:
            return PytestCapture(
                False,
                combined,
                proc.returncode,
                "historical source guard attested zero maezo imports",
            )
        if any(not path.is_relative_to(archive_root.resolve()) for path in imported_paths):
            return PytestCapture(
                False, combined, proc.returncode, "historical source import escaped archived tree"
            )
        return PytestCapture(
            True,
            combined,
            proc.returncode,
            f"{len(imported_paths)} archived maezo imports attested",
        )
...
def verify_historical_row(
    ...
    *,
    ...
    capture = _capture_historical_recipe(repo_root, edge, python_exe)
    outcome = recipe_outcome_from_capture(capture, row.test_path, node_id_regex=_RESULT_LINE_RE)
    if not outcome.ok:
        return RowVerification(
            row,
