...
adapter is the thin shim that lets Marina's `gather` node consume the generic client through the
`graph.PatientSummaryReader` Protocol shape: a single `Patient` resource read stands in for the
donor's richer "patient summary" composite (a real multi-resource summary is a follow-up, not
built here — `gather` is best-effort and never blocks routing on what it returns).
"""
