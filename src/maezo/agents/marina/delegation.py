...
async def delegate_marina_analysis(
    ...
    *,
    ...
    structured `DelegationResult` (never a raise out of the dispatcher for a TERMINAL handler
    failure). On re-delivery of the same `task_id`, `idempotent_replay=True` and the handler does
    NOT run again.
