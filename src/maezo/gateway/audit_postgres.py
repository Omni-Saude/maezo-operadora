...
class PostgresAuditSink:
    ...
    async def emit_once_on(
        ...
        THE PRECONDITION BELOW IS ENLISTMENT-ONLY, AND DELIBERATELY NOT SHARED WITH
        `emit_once_status`/`emit` (regression PR-A-AUDIT-ENLIST-001, real-engine lane of PR #375).
        Here the CALLER owns the connection, so neither the transaction nor the `search_path` is
        under this sink's control: the two checks are the only thing standing between an enlisted
        write and a chain row committed into a foreign tenant's schema. On the STANDALONE paths the
        sink acquires its own connection from its own pool, whose `setup=` pins `search_path` to
        `self._schema` on every acquire (see `_configure_connection`), and opens its own
        transaction — both facts are structural, not asserted.

        `record.tenant_id` is a DATA column (`_insert_chain_row` persists it verbatim), not the
        routing key: isolation comes from the schema. Requiring it to equal `self._tenant_id` on a
        standalone emit would be a NEW product rule — one `emit()` (the sibling write path, :273)
        does not impose and `main` never imposed — and it broke every caller whose business tenant
        differs from the schema tenant, which is exactly how the integration lane is built
        (`tests/integration/conftest.py` binds a per-run schema `it_<RUN_ID>` while the flows audit
        the business tenant). Pinned by `test_audit_postgres.py::test_emit_once_status_*` /
        `::test_emit_once_on_*`.
        """
    ...
    async def _emit_once_chained(
        ...
        the dedup lookup, the tail read, the claim and the chain INSERT — in that order, so the two
        entry points can never drift on the ordering the EFFECT-GATE guarantee rests on."""
        await conn.execute(_ADVISORY_LOCK_SQL, self._tenant_id)
