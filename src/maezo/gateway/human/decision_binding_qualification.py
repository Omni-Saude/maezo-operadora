"""ADR0049 D3/D5: authenticated six-binding qualification, not an authority issuer.
...
from maezo.portal.engine.decision import _BINDINGS, _OUTCOMES
...
class ConsumerQualification(Closed):
...
class BindingMaterial(Closed):
    ...
    @model_validator(mode="after")
    def six_only(self) -> BindingMaterial:
        e = self.entry
        ...
        if (
            ...
            or self.consumer.mode != ("phi_case_reader" if kind == "escalation" else "worker_hydration")
        ):
...
class VerifiedQualification(Closed):
    packet: QualificationPacket = Field(repr=False)
...
class QualificationVerifier:
    ...
    def _receipt(self, signed: SignedReceipt, d: AuthorityDocument, now: datetime) -> Receipt:
        r = signed.receipt
    ...
    def verify(
        self, packet: QualificationPacket, authorities: SignedAuthorities, now: datetime
    ) -> VerifiedQualification:
        try:
            packet = decode(QualificationPacket, canonical(packet))
            ...
            if f.receipt_digests != tuple(
                ...
                raise BindingUnavailableError()
            members = f.batch
            targets = [(m.process_definition_id, m.task_definition_key) for m in members]
            if (
                targets != sorted(targets)
                or len(set(targets)) != len(targets)
                or any((m.process_definition_key, m.task_definition_key) not in _BINDINGS for m in members)
    ...
    def revocation(self, value: Revocation, authorities: SignedAuthorities, now: datetime) -> Receipt:
        ...
        if r.purpose != "freeze" or r.source_ref != "decision-binding-revocation" or r.receipt_digests:
            raise BindingUnavailableError()
