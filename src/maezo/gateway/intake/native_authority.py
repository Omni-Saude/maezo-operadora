...
class PostgresAuthEffectAuthorizationSource(AuthEffectAuthorizationSource):
    ...
    async def current(
        ...
        if read:
            ...
            session_until = min(
                caller.valid_until, session.record.expires_at, session.membership.reviewed_until
            )
        ...
        async def revalidate() -> None:
            nonlocal invalid, until
            try:
                guard()
                if read:
                    if caller is None:
                        raise AuthUnavailableError()
                    current_session = await caller.revalidate()
                    until = min(
                        until,
                        caller.valid_until,
                        current_session.record.expires_at,
                        current_session.membership.reviewed_until,
                    )
                else:
                    if original is None:
                        raise AuthUnavailableError()
