...
class HumanAuthenticator(Protocol):
    """BFF port: verified identity only; never expose OAuth tokens or the HTTP client."""
...
class CognitoAuthenticator:
    ...
    async def _json(self, method: str, url: str, data: dict[str, str] | None = None) -> object:
        allowed = {
            ...
            ("POST", f"{self.settings.cognito_origin}/oauth2/token"),
        }
        if (method, url) not in allowed or (method == "GET" and data is not None):
