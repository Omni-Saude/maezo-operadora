...
Satisfies `tools/mcp_cibseven/transport.py:138`'s `CibSevenTransport` Protocol and — when the
...
(`transport.py:1052-1202`; the CI fence makes every other call site a build failure). By the time
control gets there, step 1 has ALREADY written the durable ADR-0007 claim (`:1136-1140`). A
refusal raised at `:1177` lands in the `except CibSevenError` at `:1178`, which for a STRICT
...
`transport.py:1163` — step 3, i.e. AFTER the claim. Under a future `consulta_processo` flip a
...
class GatedHistoryQueryingCibSevenTransport(GatedCibSevenTransport):
    ...
    Load-bearing: `_require_strict_gate_seams` (`transport.py:923-930`) probes the injected
