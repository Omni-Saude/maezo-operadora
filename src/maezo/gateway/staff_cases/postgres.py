...
def validate_session(row: Mapping[str, Any], *, tenant: str, issuer: str, hashed: str,
                     expected: HumanPrincipal, now: datetime) -> ResolvedHumanSession:
    try:
        ...
        if (member.audience != "staff" or member.revoked or member.tenant != tenant
                or session.issuer != issuer or member.issuer != issuer or session.secret_hash != hashed
                or session.subject != member.subject or session.principal_ref != member.principal_ref
                or session.membership_revision != member.revision or session.expires_at <= now
                or member.reviewed_until <= now or row["session_tenant"] != tenant
                or row["membership_tenant"] != tenant or row["secret_hash"] != hashed
                or row["expires_at"] != session.expires_at or row["issuer"] != member.issuer
                or row["subject"] != member.subject or row["principal_ref"] != member.principal_ref):
            raise ValueError
        principal = HumanPrincipal(schema_version=1, principal_ref=member.principal_ref, issuer=session.issuer,
            subject=session.subject, tenant=tenant, membership_revision=member.revision,
            memberships=member.memberships, session_ref=session.session_ref,
            authenticated_at=session.authenticated_at, subject_bindings=member.subject_bindings)
...
class StaffSessionLease:
    def __init__(self, engine: AsyncEngine, tenant: str, issuer: str, *, seconds: float = 5,
                 clock: Callable[[], datetime] = now_utc):
        qualify_engine(engine, seconds)
        self.engine, self.tenant, self.issuer, self.seconds, self.clock = engine, tenant, issuer, seconds, clock
    ...
    async def acquire(self, secret: str, expected: HumanPrincipal) -> AsyncIterator[SessionLock]:
        ...
        async with transaction(self.engine, self.seconds) as connection:
            row = (await connection.execute(text(
                "SELECT * FROM portal_identity.lock_external_session(:secret_hash)"
            ), {"secret_hash": hashed})).mappings().one()
            resolved = validate_session(row, tenant=self.tenant, issuer=self.issuer,
                                        hashed=hashed, expected=expected, now=self.clock())
            locked = SessionLock(resolved, min(resolved.record.expires_at, resolved.membership.reviewed_until),
...
class RelationPin:
    ...
    def __post_init__(self) -> None:
        if type(self.oid) is not int or self.oid < 1 or not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]{0,62}", self.owner):
            raise StaffCaseError("unavailable")
...
class NativeMembershipSource:
    ...
    def __init__(self, engine: AsyncEngine, scope: Scope, login: str,
                 pins: Mapping[str, RelationPin], *, seconds: float = 5,
                 clock: Callable[[], datetime] = now_utc):
        qualify_engine(engine, seconds)
    ...
    async def observe(self, principal: HumanPrincipal) -> tuple[SourceProvenance, dict[str, Any], datetime]:
        if principal.tenant != self.scope.tenant:
            raise StaffCaseError("denied")
        args = dict(tenant=self.scope.tenant, environment=self.scope.environment,
                    engine=self.scope.engine_name, incarnation=self.scope.database_incarnation,
                    principal=principal.principal_ref)
        async with transaction(self.engine, self.seconds) as connection:
            users = (await connection.execute(text("SELECT session_user::text AS actual,current_user::text AS effective"))).mappings().one()
            ...
            for name, pin in sorted(self.pins.items()):
                row = (await connection.execute(text("""
                    SELECT c.oid, c.relkind, c.relrowsecurity, c.relforcerowsecurity,
                      pg_get_userbyid(c.relowner) AS owner, pg_has_role(session_user,c.relowner,'MEMBER') AS owner_member,
                ...
                if (row["oid"] != pin.oid or row["owner"] != pin.owner or row["relkind"] != "r"
                        or row["relrowsecurity"] or row["relforcerowsecurity"] or row["owner_member"]
                        or not row["rd"] or row["wr"]):
                    raise StaffCaseError("unavailable")
            ...
            native = dict(tenant=row["tenant_"], principal_ref=row["principal_"], issuer=row["issuer_"],
                          subject=row["subject_"], revision=str(row["rev_"]), active=row["active_"],
                          valid_until=utc(valid), groups=groups)
            if (type(row["rev_"]) is not int or row["rev_"] < 0 or row["active_"] is not True
                    or type(groups) is not list or any(type(group) is not str for group in groups)
                    or member.audience != "staff" or member.state != "active"
                    or member.principal_ref != principal.principal_ref or member.issuer != principal.issuer
                    or member.subject != principal.subject or member.membership_revision != principal.membership_revision
                    or member.memberships != principal.memberships or member.subject_bindings != principal.subject_bindings
                    or native["issuer"] != principal.issuer or native["subject"] != principal.subject
