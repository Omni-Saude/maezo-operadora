...
def record_resposta_recusada(*, agent_id: str, motivo: str, response_kind: str) -> None:
    ...
    THE DEFECT THIS MEASURES. `response-v3` proibiu a negativa clinica em maiusculas e com o
    raciocinio inteiro, e o modelo passou por cima duas vezes na mesma conversa (medido em
    13/09/2026). Pedir ao modelo e' instrucao; a cerca em `_respond_llm` e' o que impede. Este
    contador e' o que diz quanto o modelo AINDA tenta — e' a medida de quanto a instrucao sozinha
    valeria, e some do radar se ninguem o emitir.

    CONTENT-FREE por construcao: `motivo` e' um dos dois rotulos fechados e `response_kind` e' o
    vocabulario fechado do grafo. O padrao exato e o texto ficam no log, nunca aqui.

    Best-effort, como `record_agent_desfecho`: um defeito de telemetria nunca pode derrubar o
    turno cuja recusa ele esta contando — e uma recusa nao contada ainda e' uma recusa aplicada.
    """
    try:
        collector = _get_metrics_collector()
        collector.agent_resposta_recusada.labels(
            agent_id=agent_id, motivo=motivo, response_kind=response_kind
        ).inc()
    except Exception:  # telemetria nunca derruba o turno cuja recusa ela conta (ver docstring)
