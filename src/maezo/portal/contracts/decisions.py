...
class DecisionReceiptResponse(PublicReceipt):
    ...
    def preserve_assignment_wire(self, handler: SerializerFunctionWrapHandler):  # type: ignore[no-untyped-def]
        # This view is v2-only, so assignment field removal does not apply. Retain
        # the model-field serializer schema rather than D5's generic dict return
        # schema; otherwise OpenAPI loses every receipt field/closed-object fence.
        return handler(self)
