...
public final class ExternalCaseReadCommand implements Command<ExternalCaseReadCommand.Result> {
  ...
  @Override public Result execute(CommandContext context){
    ...
    var db=new ExternalCaseStore(context,verified.configuration.scope(),verified.admission.statementTimeoutSeconds(),verified.configuration.engineSchema(),verified.configuration.nativeSchema());long authorityRevision=db.lock();
