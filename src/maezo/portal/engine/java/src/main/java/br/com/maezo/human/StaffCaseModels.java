...
/** Closed staff detail and complete-checkpoint list grammar. */
final class StaffCaseModels {
  ...
  static final Map<String,String> SHAPES=Map.ofEntries(
    ...
    Map.entry("decision","decision_ref:r policy_ref:r policy_revision:n policy_digest:h subject_identity_digest:h membership_revision:n resource_identity_digest:h operation:detail|list projection:projection fields:[r receipt_ref:r receipt_digest:h decision_proof:@proof observed_at:t valid_until:t state:active|revoked"),
    ...
    Map.entry("grant_entry","case_ref:case identity_digest:h grant_ref:r grant_revision:n grant_digest:h source_ref:r source_revision:n"),
    Map.entry("scope_chunk","checkpoint_ref:r generation:n chunk_index:n entries:[@grant_entry"),
    Map.entry("chunk_pin","chunk_index:n chunk_digest:h entry_count:n"),
    Map.entry("scope_checkpoint","scope:@scope checkpoint_ref:r generation:n predecessor_digest:?h principal_identity_digest:h issuer:s subject:r principal_ref:r membership_revision:n kind:authorization operations:[list chunks:[@chunk_pin total_entries:n policy_scope_ref:r policy_scope_revision:n policy_scope_digest:h observed_at:t valid_until:t coverage:complete proof:@proof"),
    Map.entry("revoke","target_kind:case_grant|scope_checkpoint target_ref:r expected_revision:n"),
    Map.entry("list","kind:authorization limit:n cursor:?r"),
    ...
    Map.entry("cursor","cursor_ref:r scope:@scope principal_identity_digest:h membership_revision:n session_ref:r operation:list query_digest:h kind:authorization checkpoint_ref:r generation:n checkpoint_digest:h case_ref:?case native_revision:?n after_ref:?r limit:n source_pins:[@readpin initial_valid_until:t"),
    Map.entry("readpin","kind:designation|identity|membership|case_grant|checkpoint|native_case|native_task|task_disclosure|source_key|policy_head|native_task_created_at ref:r revision:n digest:h valid_until:t"),
  ...
  static void requireReadCapabilities(Map<String,Object> entry){requireReadCapabilities(entry,"detail");}
  static Map<String,Object> shape(String name,Object value){
    ...
    if(name.equals("list")&&(number(m.get("limit"))<1||number(m.get("limit"))>100))throw invalid();
  ...
  static Map<String,Object> publication(Object value){
    ...
    String kind=str(r,"kind");if(!Set.of("case_grant","scope_chunk","scope_checkpoint","revoke","policy_head").contains(kind))throw invalid();
    ...
    if(kind.equals("scope_chunk")||kind.equals("scope_checkpoint")){
      if(r.get("membership_witness")==null||!"scope_complete".equals(obj(r,"proof").get("purpose")))throw denied();
    }
