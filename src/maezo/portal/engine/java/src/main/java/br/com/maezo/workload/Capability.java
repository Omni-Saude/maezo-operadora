...
public final class Capability {
  ...
  Capability(Map<String,Object> binding) {
    Json.keys(binding, "document", "digest", "source_kind", "source_worker_id", "attestations");
    document = Json.object(binding.get("document")); digest = Json.token(binding,"digest");
    Json.keys(document, "protocol", "identity", "target", "schema", "worker_id", "source_target");
  ...
  static Map<String,Map<String,Object>> schemas() {
    try (InputStream in = Capability.class.getResourceAsStream("/engine-schemas-v1.json")) {
      var root = Json.parse(Objects.requireNonNull(in).readAllBytes());
      Json.keys(root,"protocol","schemas");
      if (!"maezo.engine-schemas.v1".equals(root.get("protocol"))) throw Refused.unavailable();
