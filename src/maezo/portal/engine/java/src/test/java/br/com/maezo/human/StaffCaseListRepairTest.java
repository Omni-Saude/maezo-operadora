...
class StaffCaseListRepairTest {
  ...
  @Test void shorterInitialQ2CeilingRefusesBeforeFrozenCursorOrResponse(){
    var initial=state(SHORT,SHORT);byte[] before=Jcs.canonical(initial);var writes=new int[]{0};
    assertThrows(Rejected.class,()->{
      Instant current=StaffCaseReadCommand.normalizePinsForFrozenObservation(initial,LONG,LONG,SHORT);
      StaffCaseReadCommand.materializeCursor(value->{writes[0]++;return value;},ref->null,initial,current,null);
    });
    assertArrayEquals(before,Jcs.canonical(initial));assertEquals(0,writes[0]);
  }
  @Test void shorterCommittingSourceCeilingCannotReleaseFrozenResponse(){
    var current=state(SHORT,SHORT);byte[] before=Jcs.canonical(current);
    var result=new StaffCaseReadCommand.Result();result.frozen=new byte[]{1};result.current=()->{};
    assertThrows(Rejected.class,()->{
      StaffCaseReadCommand.normalizePinsForFrozenObservation(current,LONG,SHORT,LONG.plusSeconds(1));
      result.committed=true;
    });
    assertArrayEquals(before,Jcs.canonical(current));assertFalse(result.committed);
    assertThrows(Rejected.class,result::bytes);
  }
  @Test void equalOrLongerCurrentCeilingsPreserveFrozenSemanticBytes(){
    for(var ceilings:List.of(List.of(LONG,LONG),List.of(LONG.plusSeconds(2),LONG.plusSeconds(1)))){
      var current=state(ceilings.get(0),ceilings.get(1));
      assertEquals(StaffCaseReadCommand.currentCeiling(ceilings.get(0),ceilings.get(1)),
        StaffCaseReadCommand.normalizePinsForFrozenObservation(current,LONG,ceilings.get(0),ceilings.get(1)));
      assertTrue(list(current.get("pins")).stream().allMatch(pin->time(map(pin).get("valid_until")).equals(LONG)));}
  }
