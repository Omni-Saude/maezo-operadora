...
it("recusa recibo de conclusão que aponta para outro upload", async () => {
  const fetcher = vi.fn().mockResolvedValue(json({
    upload_ref: ref("another_upload"), disposition: "screening", document_ref: null,
  }, 202));
  expect(await client(fetcher).completeUpload(
    ref("upload"),
    { command_id: ref("command") },
    signal(),
  )).toEqual({ kind: "failure", failure: "outcome-unknown" });
});
