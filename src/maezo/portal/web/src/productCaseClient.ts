...
function validateUploadReceipt(value: unknown, expectedUploadRef?: string): UploadReceipt | null {
  if (!validWire(value, wireSchema("UploadReceipt"))) return null;
  const receipt = value as UploadReceipt;
  if (expectedUploadRef !== undefined && receipt.upload_ref !== expectedUploadRef) return null;
  ...
  return {
    ...
    completeUpload(uploadRef, submission, signal) {
      ...
      return post(
        ...
        "UploadCompletion",
        signal,
        (value) => validateUploadReceipt(value, uploadRef),
      );
    },
