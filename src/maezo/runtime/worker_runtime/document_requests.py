...
class DocumentRequestHost:
    ...
    async def _operation(self, function: Callable[[], Awaitable[_Result]], *, mutation: bool) -> _Result:
        ...
        try:
            ...
            # A bug in the producer is never absorbed into an "unavailable" refusal: the mutation
            # state still latches (the host stays fail-closed), the error surfaces as itself.
            if mutation:
