...
1 raw-handler module (T3.1 R2 — closes the gap the T3.1 phase-1 integration suites documented:
    46 of 53 `strict-xfail` markers across escalation/auth/cancel traced to this ONE missing
    module): `events` — serves the generic `operadora.events.publish` topic EVERY SP-OP-* BPMN
    uses to emit domain events. Registered via `harness.register()` (a raw `ExternalTask` handler,
