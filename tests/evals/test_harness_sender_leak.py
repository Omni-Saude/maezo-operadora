...
@pytest.mark.eval
@pytest.mark.parametrize("agent", ["fernando", "lucas"])
async def test_r6_journey_checks_actual_run_result_sender(
    agent: str, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Confere identidade do sender real; Lucas ainda detecta canario so no ACK.

    A jornada Fernando encaminha ao humano sem enviar mensagem: nao inventamos
    um ACK para ela. Seu sender vazio deve ser o mesmo retornado pelo harness.
    """
    journey = importlib.import_module(f"tests.evals.test_{agent}_journey_evals")
    case = copy.deepcopy(getattr(journey, f"{agent.upper()}_JOURNEY_CASES")[0])
    case["leak_canaries"] = [_SENDER_ONLY_CANARY]
    actual_run_case = journey.run_case
    observed: list[Any] = []
    checked: list[bool] = []

    async def observe_run(*args: Any, **kwargs: Any) -> Any:
        result = await actual_run_case(*args, **kwargs)
        assert _SENDER_ONLY_CANARY not in json.dumps(result.state, default=str)
        assert result.whatsapp is kwargs["extra_config"]["whatsapp"]
        observed.append(result.whatsapp)
        if agent == "lucas":
            assert result.whatsapp.sent, "Lucas deve produzir o ACK previsto pelo golden"
            recipient, text = result.whatsapp.sent[-1]
            result.whatsapp.sent[-1] = (recipient, text + _SENDER_ONLY_CANARY)
        else:
            assert result.whatsapp.sent == []
        return result

    def check_actual_sender(blob: Any, canaries: Any, *, sender: Any = None) -> None:
        assert observed and sender is observed[0], "a jornada omitiu o sender real do RunResult"
        checked.append(True)
        assert_no_leak(blob, canaries, sender=sender)

    monkeypatch.setattr(journey, "run_case", observe_run)
    monkeypatch.setattr(journey, "assert_no_leak", check_actual_sender)
    run_journey = getattr(journey, f"test_{agent}_journey_eval_intake_triage_handoff")
    if agent == "lucas":
        with pytest.raises(AssertionError, match="ABS violation"):
            await run_journey(case)
    else:
        await run_journey(case)
    assert checked == [True]
