...
Synthetic identifiers are deliberately LOW-ENTROPY (`t1`/`caso-001`/`pseudo-b1`/...) — gitleaks
...
def _gathered_state(**overrides: Any) -> BeatrizState:
    ...
    state = _case_state(
        business_key="FRAUDE-t1-caso-001",
        gathered=True,
...
async def test_instruct_builds_dossier_and_neutral_desfecho() -> None:
    ...
    assert dossier["business_key"] == "FRAUDE-t1-caso-001"
    assert dossier["numero_caso"] == "caso-001"
