...
def test_state_from_envelope_raises_if_a_meta_key_ever_escapes_the_input_boundary(
    ...
    """
    import maezo.agents.gustavo.delegation as delegation_module
    ...
    with pytest.raises(ValueError, match="non-input keys for Gustavo"):
        state_from_envelope(envelope)
...
async def test_handler_raises_instead_of_reporting_a_failed_start_as_success() -> None:
    ...
    with pytest.raises(StartProcessFailedError) as exc:
        await _handler(_dmn_nip(), cibseven=transporte)(_envelope())
    ...
    texto = str(exc.value)
    assert "gustavo" in texto and "SP-OP-NIP-001" in texto
    assert "pseudo-123" not in texto and "negativa_cobertura" not in texto
