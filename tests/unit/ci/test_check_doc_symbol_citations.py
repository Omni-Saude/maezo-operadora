...
def test_import_citation_to_a_missing_name_fails(tmp_path: Path) -> None:
    root = _make_fixture_repo(
        tmp_path,
        adr_body=(
            "example: `from maezo.tools.workers.widget import gone_name, still_here`\n"
        ),
        py_body="def still_here():\n    pass\n",
    )
    result = gate.run_gate(root, root / "docs" / "adr")
...
def test_disclosed_rot_is_reported_but_does_not_fail_the_gate(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    root = _make_fixture_repo(
        tmp_path,
...
def test_resolve_path_candidates_matches_bare_basename_and_suffix() -> None:
    ...
    assert gate.resolve_path_candidates("src/maezo/gateway/pep.py", tracked) == [
        "src/maezo/gateway/pep.py"
    ]
