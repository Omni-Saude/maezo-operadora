...
1. **Pure-core** tests drive `count_numbered_adr_files` / `count_all_adr_dir_files` / `evaluate`
   against synthetic `docs/adr/`-shaped trees and synthetic PLANS.md text written to a tmp path —
   RED on a stale claim (any of the 3 patterns), GREEN on a reconciled one, and non-vacuity for
   "zero claims found" (still PASS, but the explicit zero-matches signal is asserted).
2. **Real-tree** test runs the gate against the SHIPPED `PLANS.md` + `docs/adr/` and asserts it
   passes non-vacuously (>=1 claim actually checked) — the regression proof that the fix landed
   for real, not just in a fixture.
"""
...
class TestMainEndToEnd:
    ...
    def test_custom_paths_via_flags(self, tmp_path: Path) -> None:
        ...
        exit_code = main(
            ["--plans", str(plans_path), "--adr-dir", str(adr_dir)],
            repo_root=tmp_path,
        )
        assert exit_code == 0
...
class TestAgainstTheRealShippedTree:
    """Non-vacuity: the SHIPPED PLANS.md, checked against the SHIPPED docs/adr/, passes — proving
    the fix landed for real (not just in a synthetic fixture) and that at least one claim is
    actually being checked (never a vacuous 0-claims PASS on the real file)."""

    def test_shipped_plans_reconciles_with_the_shipped_adr_tree(self) -> None:
        findings, matches_checked = evaluate(_SHIPPED_PLANS.read_text(encoding="utf-8"), _SHIPPED_ADR_DIR)
        assert findings == []
        assert matches_checked >= 1  # non-vacuity: at least the two AF-06 spots are still there
