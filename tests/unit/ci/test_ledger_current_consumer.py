...
def commit(root: Path) -> str:
    ...
    subprocess.run(['git', 'commit', '-qm', 'tiny committed source'], cwd=root, check=True, capture_output=True)
...
def test_actual_v1_terminal_expectation_and_unresolved_history(tiny: Any, tmp_path: Path) -> None:
    ...
    for item in result['rows']:
        ...
        assert binding['original']['declared_hash'] == legacy.parse_row_line(item['original']['raw_line']).declared_hash
...
def test_actual_loaded_policy_swap_refuses_preimport(tiny: Any, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, fault: str) -> None:
    root, base, old, _ = tiny
    if fault == 'helper':
        monkeypatch.setattr(metadata.MetadataClassifier, 'module_metadata', lambda *a, **k: None)
    elif fault == 'classifier':
        monkeypatch.setattr(legacy, 'classify_test_admission', lambda *a, **k: metadata.TestAdmission('OFFLINE', 'forged', current.digest((root / TEST).read_bytes()), ()))
    else:
...
def test_no_caller_expected_hash_or_dataclass_plan(tiny: Any, tmp_path: Path) -> None:
    ...
    with pytest.raises(TypeError):
        current.CurrentRunner(root, tmp_path / 'proof', base=base, plan=relations.plan_current_relations(root, base))
