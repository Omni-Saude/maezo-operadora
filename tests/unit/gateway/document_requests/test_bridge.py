...
def _frozen_clock(monkeypatch):
    """`NOW` is taken at import and this module's grants expire one minute later. In a long
    collection/run the real clock overtakes them and `alive()` refuses with `denied` (the tip's
    35-minute full run failed five tests here for exactly that reason). Freeze the clock the
    models read; the two tests that advance time install their own `Clock` over this one."""
    ...
    monkeypatch.setattr(communication_models, "datetime", Frozen)
