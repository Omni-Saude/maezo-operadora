...
from maezo.gateway.human.auth_profile import Scope, SessionBinding
...
from maezo.gateway.intake.native_source_lifecycle import AuthCallerBinding, IdentitySourceReservation
...
class SQL:
    ...
    async def execute(self, statement, values=None):
        ...
        if query == "SELECT clock_timestamp()":
            ...
            "SELECT * FROM portal_intake.intake" in query
            or "SELECT intake_ref,request_digest,guide_identity_ref FROM portal_intake.intake" in query
        ):
            rows = [
                r
...
def _bind_native_reservation_source(h, *, lose_admission_ack=False):
    ...
    class Source:
        async def reserve(self, caller, *, scope, command_id, admission_ref, operation, **facts):
            assert caller.principal == PRINCIPAL and scope == SCOPE and operation == "auth.start"
            if lose_admission_ack:
                h.db.lose_ack = True
            return IdentitySourceReservation(
                reservation_ref="r" * 32,
                tenant=PRINCIPAL.tenant,
                ...
                admitted_digest=facts["admitted_digest"],
                ...
                membership_source_revision=1,
                membership_record_digest="d" * 64,
        ...
        async def verify_reservation(self, reservation):
            assert type(reservation) is IdentitySourceReservation

    async def qualified(connection):
        return None

    h.native.source, h.native.scope, h.native.qualifier = Source(), SCOPE, qualified
...
async def test_real_admission_commit_lost_ack_is_recoverable_without_payload(setup):
    ...
    # native dispatch store is configured), so the one-shot lost-ack was never consumed and fired
    # on the read below instead — and the test never reached the real defect: recovery parsed the
    # sealed v2 `ProtectedAdmissionIdentity` as a bare `HumanPrincipal`. Admit through the REAL
    # native path (caller binding + reservation), on the same fake database, so the row, the
    # native outbox and the v2 identity are really committed and the acknowledgement really lost.
    ...
    with pytest.raises(IntakeError):
        await h.intake.admit(grant, request, caller=caller)
    assert h.db.lose_ack is False, "the admission never reached its commit"
    key = (PRINCIPAL.tenant, REF)
    assert key in h.db.intakes and key in h.db.outbox and key in h.db.identities
    sealed = h.db.identities[key]
    unsealed = h.native.unseal("identity", REF, sealed["key"], sealed["nonce"], sealed["ciphertext"])
    assert unsealed["schema"] == "human-auth-admission-identity.v2"
    h.db.queries.clear()
