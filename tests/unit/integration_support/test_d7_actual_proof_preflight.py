...
# WP-C078 port adaptation: machine-local toolkit packet (not in the repository at any ref);
# skip the module at collection where it is absent instead of failing it. Where it is present
# every assertion below runs unchanged against the packet.
if not PACKET.is_dir():
    pytest.skip(f"D7 preflight toolkit packet absent: {PACKET}", allow_module_level=True)
