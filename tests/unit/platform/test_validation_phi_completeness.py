...
class TestBuckets:
    ...
    def test_the_occurrence_count_is_pinned(self, live_sweep: Sweep) -> None:
        """332 names over 1643 occurrences — the number the ledger row quotes.
        ...
        1638 -> 1643 via PERSP-PAGTO-STALE-CONTAS-ASSUMPTION's five entries above
        (331 -> 332 names, `lastro_decisor_id` the only genuinely new one).
        """
        expected_names = _BASELINE_NAMES + sum(delta.name_delta for delta in CORPUS_DELTA_LOG)
        expected_refs = _BASELINE_OCCURRENCES + sum(delta.occurrence_delta for delta in CORPUS_DELTA_LOG)
        assert len(live_sweep.names) == expected_names == 332
        assert len(live_sweep.refs) == expected_refs == 1643
