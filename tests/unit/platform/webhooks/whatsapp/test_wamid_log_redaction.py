...
async def test_dispatch_turn_started_telemetry_carries_the_pseudonym_never_the_raw_wamid(
    ...
    with structlog.testing.capture_logs() as logs:
        with pytest.raises(RuntimeError, match="turno explodiu"):
...
def _settings() -> WhatsAppWebhookSettings:
    return WhatsAppWebhookSettings(
        app_secret=APP_SECRET, verify_token=VERIFY_TOKEN, tenant_id=_TENANT
    )
...
def _guard() -> WhatsAppDedupGuard:
    return WhatsAppDedupGuard(
        registry=FakeDedupRegistry(), pseudonymizer=_pseudonymizer(), tenant=_TENANT
    )
...
#: confianca no nome.
_SANITIZERS = frozenset({"log_safe_message_id", "hash_message_id", "pseudonym", "inbound_key", "outbound_key"})
