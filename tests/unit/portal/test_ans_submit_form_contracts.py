...
CASES = [
    ("UT_RevisarEnvio", "ans_revisao", {"decisao_envio": "APROVAR_ENVIO"}),
    ("UT_RevisarEnvio", "ans_revisao", {"decisao_envio": "CORRIGIR_PENDENCIA"}),
    (
        "UT_RevisarEnvio",
        "ans_revisao",
        {"decisao_envio": "ADIAR_ENVIO", "justificativa_adiamento": "Human basis"},
    ),
    ("UT_RevisarEnvioJuridico", "ans_revisao", {"decisao_envio": "APROVAR_ENVIO"}),
    ("UT_RevisarEnvioJuridico", "ans_revisao", {"decisao_envio": "CORRIGIR_PENDENCIA"}),
    (
        "UT_RevisarEnvioJuridico",
        "ans_revisao",
        {"decisao_envio": "ADIAR_ENVIO", "justificativa_adiamento": "Human basis"},
    ),
    ("UT_CoordenacaoEnvioAssume", "ans_coordenacao", {"decisao_envio": "APROVAR_ENVIO"}),
    (
        "UT_CoordenacaoEnvioAssume",
        "ans_coordenacao",
        {"decisao_envio": "ADIAR_ENVIO", "justificativa_adiamento": "Human basis"},
    ),
    (
        ...
        "ans_pendencia",
        {"dataset_complete": False, "schema_valid": False, "lgpd_anonimizado": False},
    ),
    (
        "UT_CorrigirPendenciaEnvio",
        "ans_pendencia",
        {"dataset_complete": False, "schema_valid": False, "lgpd_anonimizado": True},
    ),
    (
        "UT_CorrigirPendenciaEnvio",
        "ans_pendencia",
        {"dataset_complete": False, "schema_valid": True, "lgpd_anonimizado": False},
    ),
    (
        "UT_CorrigirPendenciaEnvio",
        "ans_pendencia",
        {"dataset_complete": False, "schema_valid": True, "lgpd_anonimizado": True},
    ),
    (
        "UT_CorrigirPendenciaEnvio",
        "ans_pendencia",
        {"dataset_complete": True, "schema_valid": False, "lgpd_anonimizado": False},
    ),
    (
        "UT_CorrigirPendenciaEnvio",
        "ans_pendencia",
        {"dataset_complete": True, "schema_valid": False, "lgpd_anonimizado": True},
    ),
    (
        "UT_CorrigirPendenciaEnvio",
        "ans_pendencia",
        {"dataset_complete": True, "schema_valid": True, "lgpd_anonimizado": False},
    ),
    (
        "UT_CorrigirPendenciaEnvio",
        "ans_pendencia",
        {"dataset_complete": True, "schema_valid": True, "lgpd_anonimizado": True},
    ),
    ("UT_TratarNack", "ans_nack", {"decisao_nack": "retransmitir_manual"}),
    ("UT_TratarNack", "ans_nack", {"decisao_nack": "registrar_falha"}),
]
BINDINGS = [
    ("UT_RevisarEnvio", "ans_revisao"),
    ("UT_RevisarEnvioJuridico", "ans_revisao"),
    ("UT_CoordenacaoEnvioAssume", "ans_coordenacao"),
    ("UT_CorrigirPendenciaEnvio", "ans_pendencia"),
    ("UT_TratarNack", "ans_nack"),
]
INPUTS = {
...
@pytest.mark.parametrize("task,form", BINDINGS[:3])
@pytest.mark.parametrize("bad", [None, "", " \t", 1, True, [], {}])
def test_deferral_requires_human_basis(task: str, form: str, bad: object) -> None:
    with pytest.raises(ValidationError):
        TaskDecision.model_validate(
            command(task, form, {"decisao_envio": "ADIAR_ENVIO", "justificativa_adiamento": bad})
        )


@pytest.mark.parametrize("key", ["dataset_complete", "schema_valid", "lgpd_anonimizado"])
@pytest.mark.parametrize("bad", [None, 0, 1, "true", "false", [], {}])
def test_correction_is_explicit_strict_human_confirmation(key: str, bad: object) -> None:
    data = dict(dataset_complete=True, schema_valid=True, lgpd_anonimizado=True)
    with pytest.raises(ValidationError):
        TaskDecision.model_validate(command("UT_CorrigirPendenciaEnvio", "ans_pendencia", {**data, key: bad}))
    data.pop(key)
    with pytest.raises(ValidationError):
        TaskDecision.model_validate(command("UT_CorrigirPendenciaEnvio", "ans_pendencia", data))


def test_coordination_cannot_borrow_regular_review_correction_outcome() -> None:
    with pytest.raises(ValidationError):
        TaskDecision.model_validate(
            command("UT_CoordenacaoEnvioAssume", "ans_coordenacao", {"decisao_envio": "CORRIGIR_PENDENCIA"})
        )


@pytest.mark.parametrize("task,form", BINDINGS)
def test_no_cross_task_inputs(task: str, form: str) -> None:
    own = next(c[2] for c in CASES if c[:2] == (task, form))
    for other_form in INPUTS:
        if other_form == form:
            continue
        payload = command(task, form, own)
        payload["inputs"] = {"kind": other_form, **next(c[2] for c in CASES if c[1] == other_form)}
        with pytest.raises(ValidationError):
            TaskDecision.model_validate_json(json.dumps(payload))
