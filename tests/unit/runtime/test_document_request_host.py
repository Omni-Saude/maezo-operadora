...
async def test_programming_error_in_an_operation_is_re_raised_unconverted_and_latches(host_case):
    """PR-A landing repair (broad-except fence): a bug inside a producer is never absorbed into a
    `FetchUnavailableError`; it surfaces as itself, the mutation state still latches (the host
    stays fail-closed for further mutations) and external failures keep converting."""
    c = host_case

    async def bug():
        raise TypeError("synthetic private bug")

    with pytest.raises(TypeError, match="synthetic private bug"):
        await c.host._operation(bug, mutation=True)
    assert c.host._mutation_failed is True

    async def external():
        raise OSError("synthetic private failure")

    with pytest.raises(FetchUnavailableError):
        await c.host._operation(external, mutation=False)
    with pytest.raises(FetchUnavailableError):
        await c.host._operation(external, mutation=True)
