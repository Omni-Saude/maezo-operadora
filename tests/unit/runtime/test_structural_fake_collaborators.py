...
# Real test files in THIS tree whose doubles are correct external collaborators. PR-A lands
# `test_kafka_client.py` (botocore `URLLib3Session` replaced from a `@pytest.fixture`); the two
# files the train listed here (`tests/unit/ci/test_ecs_portal_deployment.py`,
# `tests/unit/portal/test_tls_failure_oracle.py`) belong to PR-C (S6a/S8) and are re-added there.
@pytest.mark.parametrize("relative", ["tests/unit/gateway/test_kafka_client.py"])
...
# --- monkeypatch injected into a `@pytest.fixture`; replaced collaborator resolved by identity ---

_FIXTURE_HTTP = """import http.client as wire
import pytest
class Specimen:
    def close(self): pass
@pytest.fixture
def install(monkeypatch):
    monkeypatch.setattr(wire, "HTTPConnection", Specimen)
"""
_REEXPORTED_URLLIB3 = """import pytest
...
def wire(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(m, "URLLib3Session", Specimen)
"""


@pytest.mark.parametrize("source", [_FIXTURE_HTTP, _REEXPORTED_URLLIB3])
@pytest.mark.parametrize("name", ["Renamed", "FakeDmn", "RegionalTransport"])
def test_pytest_fixture_receiving_monkeypatch_proves_the_receiver(source, name):
    assert fence._achados_estruturais_em_fonte(source.replace("Specimen", name), "arbitrary.py") == []


def test_reexported_botocore_session_is_resolved_by_object_identity_not_name():
    assert fence._metodos_do_colaborador_substituido("maezo.gateway.kafka_client", "URLLib3Session") == {
        "send",
        "close",
    }
    assert fence._metodos_do_colaborador_substituido("http.client", "HTTPConnection") == {"close"}


@pytest.mark.parametrize(
    ("alvo", "atributo"),
    [
        ("maezo.gateway.kafka_client", "AIOKafkaProducer"),  # real object, not a collaborator in the table
        ("maezo.gateway.kafka_client", "does_not_exist"),  # attribute missing
        ("maezo.gateway.does_not_exist_module", "URLLib3Session"),  # import fails
        ("botocore.httpsession", "URLLib3Session"),  # direct table hit needs no import
        ("os", "URLLib3Session"),  # never imports outside maezo.* — proves nothing
    ],
)
def test_replaced_object_identity_is_fail_closed(alvo, atributo):
    expected = {"send", "close"} if alvo == "botocore.httpsession" else set()
    assert fence._metodos_do_colaborador_substituido(alvo, atributo) == expected


def test_replacing_a_non_collaborator_attribute_does_not_suppress_findings():
    source = _REEXPORTED_URLLIB3.replace('"URLLib3Session"', '"AIOKafkaProducer"')
    findings = fence._achados_estruturais_em_fonte(source, "arbitrary.py")
    assert findings and all("Specimen" in finding for finding in findings)


def test_fixture_named_monkeypatch_is_a_shadow_not_an_injection():
    source = _FIXTURE_HTTP.replace("@pytest.fixture\n", '@pytest.fixture(name="monkeypatch")\n')
    assert fence._achados_estruturais_em_fonte(source, "arbitrary.py")
