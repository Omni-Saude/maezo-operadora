...
# R-173 applies before pre-shaped passthrough too. These are representation checks,
# not financial limits: signed zero/negative Long values are valid on this wire.
# Semantics are main's `declared_long_variable` (#364): a non-int / non-int64 value or a
# CONFLICTING wire type is refused; an Integer/untyped envelope is UPGRADED to Long
# (see `_UPGRADED_DECLARED_LONG` below and `test_preshaped_financial_engine_vars.py`).
_BAD_DECLARED_LONG = [
    True,
    False,
    1.0,
...
# An Integer or untyped envelope carrying exact int64 centavos is not a defect: the value is
# preserved and the wire type becomes Long (R-173 upgrade, never a magnitude-typed Integer).
_UPGRADED_DECLARED_LONG = [
...
@pytest.mark.parametrize("value", _UPGRADED_DECLARED_LONG)
@pytest.mark.parametrize("mapper", range(3))
def test_declared_long_upgrades_integer_or_untyped_envelope_to_long(value, mapper):
    assert _production_maps(value)[mapper]() == {"value": 100, "type": "Long"}
...
def test_declared_long_preserves_exact_signed_values_and_valid_shape(value, mapper):
    ...
    # main's mapper returns an equal, detached envelope (the caller's dict is never mutated).
    assert _production_maps(shaped)[mapper]() == shaped
...
@pytest.mark.parametrize("value", _UPGRADED_DECLARED_LONG)
async def test_public_transports_upgrade_integer_envelope_to_long_before_effect_post(path, value):
    posts = []
    await _exercise_declared_long_transport(path, value, posts)
    assert len(posts) == 1
    field = (
    ...
    assert posts[0][field][_VARIAVEL] == {"value": 100, "type": "Long"}
...
@pytest.mark.parametrize("wire", ["Long", "Integer", None, "float"])
async def test_computed_worker_output_is_typed_long_before_harness_effect(wire):
    """A worker's pre-shaped R-173 envelope reaches `/complete` as Long whether it was shaped
    Long, Integer or untyped (upgrade, value untouched); a non-integer value never reaches the
    engine — the harness reports a non-retryable failure without variables."""
    import json
    ...
    async def producer(task):
        ...
        if wire == "float":
            result[_VARIAVEL] = {"value": float(result[_VARIAVEL]), "type": "Long"}
        elif wire is None:
            result[_VARIAVEL] = {"value": result[_VARIAVEL]}
        else:
            result[_VARIAVEL] = {"value": result[_VARIAVEL], "type": wire}
        return result
