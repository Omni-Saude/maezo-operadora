...
| `python -m tools.staff_materials.approver sign-admission ...` | **aprovador** | **máquina dele** | assina o `portal-read-admission.v1` (T1.7) no domínio `maezo/portal-read-admission/v1\0`. Com o bloco `human` (tarefas, H1/H4) exige `--catalog C` e confere o catálogo: SHA-256 = `catalog_digest`, uma entrada `human` por tarefa, `identity_policy` = `opaque_task_id_policy`, política da classificação = `disclosure_policy`, grupos dentro do domínio da entrada. Em dev, `task_id_format=uuid` (a imagem `Dockerfile.human` gera UUID) |
